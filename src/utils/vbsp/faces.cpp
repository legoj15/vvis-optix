...
int GetVertexnum(Vector &in) {
  ...
  // emit a vertex
  if (!g_bCountVertsOnly && numvertexes >= 65536)
    Error("Too many unique verts for BSP format, max = 65536\n");
...
int GetVertexnum(Vector &v) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (v[i] < MIN_COORD_INTEGER || v[i] > MAX_COORD_INTEGER)
      Error("GetVertexnum: outside world, vertex %.1f %.1f %.1f", v.x, v.y,
            v.z);
  }
...
void FaceFromSuperverts(face_t **pListHead, face_t *f, int base) {
  ...
  while (remaining >
         MAXEDGES) { // must split into two faces, because of vertex overload
    c_faceoverflows++;
  ...
  for (i = 0; i < w->numpoints; i++) {
    if (noweld) { // make every point unique
      if (!g_bCountVertsOnly && numvertexes >= 65536)
        Error("Too many unique verts for BSP format, max = 65536\n");
      if (numvertexes == MAX_MAP_VERTS)
        Error("Too many unique verts, max = %d (map has too much brush "
              "geometry)\n",
              MAX_MAP_VERTS);
...
#if 0
{
  ...
  for (x = x1; x <= x2; x++) {
    for (y = y1; y <= y2; y++) {
      for (vnum = hashverts[y * HASH_SIZE + x]; vnum;
           vnum = vertexchain[vnum]) {
...
void TestEdge(vec_t start, vec_t end, int p1, int p2, int startvert) {
  ...
  if (numsuperverts >= MAX_SUPERVERTS)
    Error("Edge with too many vertices due to t-junctions.  Max %d verts along "
          "an edge!\n",
          MAX_SUPERVERTS);
...
void Triangulate_r(CUtlVector<int> &out, const CUtlVector<int> &inIndices,
                   const CUtlVector<face_vert_table_t> &poly) {
  ...
  // check each pair of verts and see if they are diagonal (not on a shared
  // edge) if so, split & recurse
  for (int i = 0; i < inIndices.Count(); i++) {
    int count = inIndices.Count();

    // i + count is myself, i + count-1 is previous, so we need to stop at
    // i+count-2
...
void FixFaceEdges(face_t **pList, face_t *f) {
  ...
  if (f->badstartvert && entity_num == 0) {
    ...
    if (!g_bCountVertsOnly &&
        (g_numprimitives > 32768 || g_numprimindices > 65536)) {
      Error("Too many t-junctions for BSP format! (%d prims, max 32768 :: %d "
            "indices, "
            "max 65536)\n",
            g_numprimitives, g_numprimindices);
    }
    if (g_numprimitives > MAX_MAP_PRIMITIVES ||
        g_numprimindices > MAX_MAP_PRIMINDICES) {
      Error("Too many t-junctions to fix up! (%d prims, max %d :: %d indices, "
            "max %d)\n",
            g_numprimitives, MAX_MAP_PRIMITIVES, g_numprimindices,
            MAX_MAP_PRIMINDICES);
    }
...
face_t *AllocFace(void) {
  ...
  // Placement-new the CUtlVector so its internal state is valid
  new (&f->mergedSideIds) CUtlVector<int>();
...
void FreeFace(face_t *f) {
  if (f->w)
    FreeWinding(f->w);
  // Explicitly destroy the CUtlVector before free()
  f->mergedSideIds.~CUtlVector<int>();
  free(f);
  c_faces--;
}
  ...
  if (!noshare) {
    ...
    for (int i = 0; i < theList.Size(); i++) {
      ...
      if (v1 == edge->v[1] && v2 == edge->v[0] &&
          edgefaces[iEdge][0]->contents == f->contents) {
...
bool OverlaysAreEqual(face_t *f1, face_t *f2) {
  ...
  if (f1->originalface->aOverlayIds.Count() !=
      f2->originalface->aOverlayIds.Count())
    return false;
...
face_t *TryMerge(face_t *f1, face_t *f2, Vector &planenormal) {
  ...
  // Accumulate all original VMF side IDs from both parents
  newf->mergedSideIds.RemoveAll();
  newf->mergedSideIds.AddVectorToTail(f1->mergedSideIds);
  newf->mergedSideIds.AddVectorToTail(f2->mergedSideIds);
...
static bool AssignBottomWaterMaterialToFace(face_t *f) {
  ...
  if (!GetValueFromPatchedMaterial(pMaterialName, "$bottommaterial",
                                   pBottomMatName, 512)) {
    if (!Q_stristr(pMaterialName, "nodraw") &&
        !Q_stristr(pMaterialName, "toolsskip")) {
  ...
  for (j = 0; j < 2; j++) {
    for (k = 0; k < 4; k++) {
      newTexInfo.textureVecsTexelsPerWorldUnits[j][k] =
          pTexInfo->textureVecsTexelsPerWorldUnits[j][k];
      newTexInfo.lightmapVecsLuxelsPerWorldUnits[j][k] =
          pTexInfo->lightmapVecsLuxelsPerWorldUnits[j][k];
    }
  }
...
face_t *FaceFromPortal(portal_t *p, int pside) {
  ...
  // Seed the merged side ID list with this face's original VMF side ID
  f->mergedSideIds.AddToTail(side->id);
  ...
  if (entity_num != 0) {
    ...
    // inside water faces can be flipped because they are generated on the
    // inside of the brush
  ...
  deltaContents =
      VisibleContents(p->nodes[!pside]->contents ^ p->nodes[pside]->contents);

  // don't show insides of windows or grates
  if (((p->nodes[pside]->contents & CONTENTS_WINDOW) &&
       deltaContents == CONTENTS_WINDOW) ||
      ((p->nodes[pside]->contents & CONTENTS_GRATE) &&
       deltaContents == CONTENTS_GRATE)) {
    FreeFace(f);
    return NULL;
  }
  ...
  // etc.
  if ((p->nodes[pside]->contents & CONTENTS_WATER) &&
      deltaContents == CONTENTS_WATER) {
...
//-----------------------------------------------------------------------------
int AddWindingToPrimverts(const winding_t *w, unsigned short *pIndices,
  ...
  for (int i = 0; i < w->numpoints; i++) {
    ...
    if (j >= vertStart + vertCount) {
      ...
      vertCount++;
      g_numprimverts++;
      if (!g_bCountVertsOnly && g_numprimverts > 65536) {
        Error("Exceeded max water verts for BSP format! (%d>65536)\n",
              (int)g_numprimverts);
      }
      if (g_numprimverts > MAX_MAP_PRIMVERTS) {
        Error("Exceeded max water verts.\nIncrease surface subdivision size or "
              "lower your subdivision size in vmt files! (%d>%d)\n",
...
// UNDONE: Should split this function into subdivide and primitive building
...
static void SubdivideFaceBySubdivSize(face_t *f, float subdivsize) {
  ...
  // garymcthack - this assumes a surface of constant z for now (for water). .
  ...
  for (i = 0; i < xSteps * ySteps; i++) {
    ...
    unsigned short *pIndices = (unsigned short *)_alloca(
        windings[i]->numpoints * sizeof(unsigned short));
    // find indices for the verts.
    newPrim.vertCount = AddWindingToPrimverts(
        windings[i], pIndices, newPrim.firstVert, newPrim.vertCount);

    // Now that we have indices for the verts, fan-tesselate the polygon and
    // spit out tris.
  ...
  Stripify(triListIndices.Size() / 3, triListIndices.Base(),
  ...
  for (i = 0; i < numTristripIndices; i++) {
    ...
    newPrim.indexCount++;
    g_numprimindices++;
    if (!g_bCountVertsOnly && g_numprimindices > 65536) {
      Error("Exceeded max water indices for BSP format! (%d>65536)\n",
            g_numprimindices, 65536);
    }
...
#endif
  g_numprimitives++; // don't increment until we get here and are sure that we
                     // have a primitive.
  if (!g_bCountVertsOnly && g_numprimitives > 32768) {
    Error("Exceeded max water primitives for BSP format! (%d>32768)\n",
          (int)g_numprimitives);
  }
  if (g_numprimitives > MAX_MAP_PRIMITIVES) {
