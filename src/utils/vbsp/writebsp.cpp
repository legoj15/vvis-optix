...
// NOTE: This is a global used to link faces back to the tree node/portals they
// came from it's used when filling water volumes
node_t *dfacenodes[MAX_MAP_FACES];

// Side-ID metadata: defined in bsplib.cpp, accumulated here during EmitFace
extern CUtlVector<dfacesideid_index_t> g_FaceSideIdIndex;
extern CUtlVector<int> g_FaceSideIdData;
...
void EmitMarkFace(dleaf_t *leaf_p, face_t *f) {
  ...
  if (i == numleaffaces) {
    // In normal mode, enforce the BSP format limit (engine uses 16-bit
    // indices). In countverts mode, use the expanded array limit for counting.
    if (!g_bCountVertsOnly && numleaffaces >= 65536)
      Error("Too many leaffaces for BSP format, max = 65536\n");
...
void EmitLeaf(node_t *node) {
  ...
  if (nummodels == 0) {
    ...
    // the same cluster and parts of the code like ivp.cpp's
    // ConvertWaterModelToPhysCollide won't work.
    leaf_p->cluster = -1;
  }
...
int CreateOrigFace(face_t *f) {
  ...
  for (i = 0; i < pWinding->numpoints; i++) {
    ...
    for (j = firstmodeledge; j < numedges; j++) {
      if ((eIndex[0] == dedges[j].v[1]) && (eIndex[1] == dedges[j].v[0]) &&
        ...
        if (numsurfedges >= MAX_MAP_SURFEDGES)
          Error("Too much brush geometry in bsp, numsurfedges == "
                "MAX_MAP_SURFEDGES");
...
void EmitFace(face_t *f, qboolean onNode) {
  ...
  //
  // In normal mode, enforce the BSP format limit (engine uses 16-bit indices).
  // In countverts mode, use the expanded array limit for counting.
  if (!g_bCountVertsOnly && numfaces >= 65536)
    Error("Too many faces for BSP format, max = 65536");
  if (numfaces >= MAX_MAP_FACES)
    Error("Too many faces in map, max = %d", MAX_MAP_FACES);
  df = &dfaces[numfaces];

  // Save the correlation between dfaces and faces -- since dfaces doesnt have
  // worldcraft face id
  dfaceids.AddToTail();
  dfaceids[numfaces].hammerfaceid = f->originalface->id;

  // Accumulate merged side IDs for the new lump
  if (g_bEmitSideIds) {
    dfacesideid_index_t idx;
    idx.firstId = g_FaceSideIdData.Count();
    if (f->mergedSideIds.Count() > 0) {
      idx.numIds = f->mergedSideIds.Count();
      for (int si = 0; si < f->mergedSideIds.Count(); si++) {
        g_FaceSideIdData.AddToTail(f->mergedSideIds[si]);
      }
    } else {
      // Fallback: use the single originalface id
      idx.numIds = 1;
      g_FaceSideIdData.AddToTail(f->originalface->id);
    }
    g_FaceSideIdIndex.AddToTail(idx);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Emit all of the faces stored at the leaves (faces from detail
// brushes)
//-----------------------------------------------------------------------------
...
// This will generate a scratchpad file with the level's geometry in it and the
// noshadow faces drawn red. #define SCRATCHPAD_NO_SHADOW_FACES
...
void MarkNoShadowFaces() {
  ...
  for (int iFace = 0; iFace < numfaces; iFace++) {
    ...
    if (!pFace->AreDynamicShadowsEnabled()) {
      ScratchPad_DrawFace(g_pPad, pFace, iFace, CSPColor(1, 0, 0, 1),
                          Vector(1, 0, 0));
      ScratchPad_DrawFace(g_pPad, pFace, iFace, CSPColor(1, 0, 0, 1),
                          Vector(-1, 0, 0));
      ScratchPad_DrawFace(g_pPad, pFace, iFace, CSPColor(1, 0, 0, 1),
...
int FindMatchingBrushSideTexinfo(int sideIndex, const texinfomap_t *pMap) {
  ...
  // find one with the same flags & surfaceprops (even if the texture name is
  // different)
  ...
  for (int j = 0; j < texinfo.Count(); j++) {
    if (pMap[j].refCount > 0 && texinfo[j].flags == sideTexFlags &&
...
void CompactTexdataArray(texdatamap_t *pMap) {
  ...
  oldStringData.CopyArray(g_TexDataStringData.Base(),
                          g_TexDataStringData.Count());
  g_TexDataStringData.RemoveAll();
  CUtlVector<int> oldStringTable;
  oldStringTable.CopyArray(g_TexDataStringTable.Base(),
                           g_TexDataStringTable.Count());
  ...
  for (int i = 0; i < oldTexData.Count(); i++) {
    ...
    const char *pString =
        &oldStringData[oldStringTable[oldTexData[i].nameStringTableID]];
...
void CompactTexinfos() {
  ...
  for (i = 0; i < texinfo.Count(); i++) {
    ...
    // const char *pName = TexDataStringTable_GetString(
    // dtexdata[texinfo[i].texdata].nameStringTableID );
  }
  ...
  for (i = 0; i < numleafwaterdata; i++) {
    if (dleafwaterdata[i].surfaceTexInfoID >= 0) {
      dleafwaterdata[i].surfaceTexInfoID =
          texinfoMap[dleafwaterdata[i].surfaceTexInfoID].outputIndex;
    }
  }
  // remap world lights
  for (i = 0; i < *pNumworldlights; i++) {
  ...
  for (i = 0; i < g_nWaterOverlayCount; i++) {
    if (g_WaterOverlays[i].nTexInfo >= 0) {
      g_WaterOverlays[i].nTexInfo =
          texinfoMap[g_WaterOverlays[i].nTexInfo].outputIndex;
    }
  }

  Msg("Reduced %d texinfos to %d\n", oldCount, texinfo.Count());
  Msg("Reduced %d texdatas to %d (%d bytes to %d)\n", oldTexdataCount,
      numtexdata, oldTexdataString, g_TexDataStringData.Count());
...
void SetLightStyles(void) {
  ...
  for (i = 1; i < num_entities; i++) {
    ...
    if (pCurrentStyle) {
      ...
      if (oldStyle != 0) {
        // save off the default style so the game code can make a switchable
        // copy of it
...
// We can't calculate this properly until vvis (since we need vis to do this),
// so we set to zero everywhere by default.
...
void DiscoverMacroTextures() {
  ...
  for (int iFace = 0; iFace < numfaces; iFace++) {
    ...
    const char *pMaterialName =
        &g_TexDataStringData[g_TexDataStringTable[pTexData->nameStringTableID]];

    MaterialSystemMaterial_t hMaterial =
        FindMaterial(pMaterialName, NULL, false);
    ...
    if (pMacroTextureName) {
      ...
      g_FaceMacroTextureInfos[iFace].m_MacroTextureNameID =
...
// Make sure that we have a water lod control entity if we have water in the
// map.
void EnsurePresenceOfWaterLODControlEntity(void) {
  ...
  // None found, add one.
  Warning("Water found with no water_lod_control entity, creating a default "
          "one.\n");
...
void EndBSPFile(void) {
  ...
  {
    // phys collision needs dispinfo to operate (needs to generate phys
    // collision for displacement surfs)
    EmitPhysCollision();
  }
  ...
  // Emit side ID metadata lumps if requested
  if (g_bEmitSideIds && g_FaceSideIdIndex.Count() > 0) {
    Msg("Emitting side ID metadata: %d faces, %d total IDs\n",
        g_FaceSideIdIndex.Count(), g_FaceSideIdData.Count());
    // Write directly via AddLump in WriteBSPFile's output stream.
    // We stash the data in globals and write them alongside the other lumps.
  }
...
//-----------------------------------------------------------------------------
static void AddNodeToBounds(int node, CUtlVector<int> &skipAreas, Vector &mins,
                            Vector &maxs) {
  // not a leaf
  if (node >= 0) {
...
// in the 3d skybox
//-----------------------------------------------------------------------------
bool IsBoxInsideWorld(int node, CUtlVector<int> &skipAreas,
...
//-----------------------------------------------------------------------------
void AddDispsToBounds(int nHeadNode, CUtlVector<int> &skipAreas,
