...
static bool SideIsNotDispAndHasDispMaterial(int iSide) {
  ...
  // If it's a displacement, then it's fine to have a displacement-only
  // material.
  if (pSide->pMapDisp) {
    return false;
  }
...
//-----------------------------------------------------------------------------
static void GeneratePatchedMaterialName(const char *pMaterialName,
                                        char *pBuffer, int nMaxLen) {
  int nLen = Q_snprintf(pBuffer, nMaxLen, "maps/%s/%s_wvt_patch", mapbase,
                        pMaterialName);

  Assert(nLen < TEXTURE_NAME_LENGTH - 1);
  if (nLen >= TEXTURE_NAME_LENGTH - 1) {
    Error("Generated worldvertextransition patch name : %s too long! (max = "
          "%d)\n",
...
void CreateWorldVertexTransitionPatchedMaterial(
    const char *pOriginalMaterialName, const char *pPatchedMaterialName) {
  // Register the patch->original mapping so FindOriginalMaterial() can trace
  // back to the original WVT material (needed for %detailtype, surfaceprop,
  // etc.)
  ...
  if (kv) {
    ...
    // If we didn't want a basetexture on the first texture in the blend, we
    // don't want an envmap at all.
...
int CreateBrushVersionOfWorldVertexTransitionMaterial(int originalTexInfo) {
  ...
  if (!bHasTexData) {
    ...
    CreateWorldVertexTransitionPatchedMaterial(pOriginalMaterialName,
                                               patchedMaterialName);
...
void WorldVertexTransitionFixup(void) {
  ...
  for (i = 0; i < g_MainMap->nummapbrushes; i++) {
    sideList[g_MainMap->mapbrushes[i].entitynum].brushSideCount +=
        g_MainMap->mapbrushes[i].numsides;
  }
  int curSide = 0;
  for (i = 0; i < g_MainMap->num_entities; i++) {
  ...
  for (int iSide = 0; iSide < g_MainMap->nummapbrushsides; ++iSide) {
    ...
    while (currentEntity < g_MainMap->num_entities - 1 &&
           iSide > sideList[currentEntity].firstBrushSide +
                       sideList[currentEntity].brushSideCount) {
      currentEntity++;
    }

    pSide->texinfo =
        CreateBrushVersionOfWorldVertexTransitionMaterial(pSide->texinfo);
  }
}