...
// this will output glview files for the given brushmodel.  Brushmodel 1 is the
// world, 2 is the first brush entity, etc.
...
struct side_t {
  ...
  CUtlVector<int>
      aWaterOverlayIds; // List of water overlays that reside on this side.
  bool m_bDynamicShadowsEnabled; // Goes into
                                 // dface_t::SetDynamicShadowsEnabled().
};

struct mapbrush_t {
  int entitynum;
  int brushnum;
  int id; // The unique ID of this brush in the editor, used for reporting
          // errors.
  int contents;
  Vector mins, maxs;
  int numsides;
...
struct face_t {
  ...
  int numPrims;
  unsigned int smoothingGroups;
  CUtlVector<int> mergedSideIds; // all VMF side IDs merged into this face
};

extern qboolean g_bEmitSideIds;
...
extern qboolean g_bCountVertsOnly;
...
public:
  ...
  ChunkFileResult_t LoadConnectionsKeyCallback(const char *szKey,
                                               const char *szValue,
                                               LoadEntity_t *pLoadEntity);
  ChunkFileResult_t LoadSolidCallback(CChunkFile *pFile,
                                      LoadEntity_t *pLoadEntity);
  ...
  static bool DeterminePath(const char *pszBaseFileName,
  ...
  void MergePlanes(entity_t *pInstanceEntity, CMapFile *Instance,
                   Vector &InstanceOrigin, QAngle &InstanceAngle,
                   matrix3x4_t &InstanceMatrix);
  void MergeBrushes(entity_t *pInstanceEntity, CMapFile *Instance,
                    Vector &InstanceOrigin, QAngle &InstanceAngle,
                    matrix3x4_t &InstanceMatrix);
  void MergeBrushSides(entity_t *pInstanceEntity, CMapFile *Instance,
                       Vector &InstanceOrigin, QAngle &InstanceAngle,
                       matrix3x4_t &InstanceMatrix);
  void ReplaceInstancePair(epair_t *pPair, entity_t *pInstanceEntity);
  void MergeEntities(entity_t *pInstanceEntity, CMapFile *Instance,
                     Vector &InstanceOrigin, QAngle &InstanceAngle,
                     matrix3x4_t &InstanceMatrix);
  void MergeOverlays(entity_t *pInstanceEntity, CMapFile *Instance,
                     Vector &InstanceOrigin, QAngle &InstanceAngle,
                     matrix3x4_t &InstanceMatrix);
...
int TexinfoForBrushTexture(plane_t *plane, brush_texture_t *bt,
                           const Vector &origin);
...
bool LoadStudioModel(char const *pFileName, char const *pEntityType,
                     CUtlBuffer &buf);
...
int BrushBspBoxOnPlaneSide(const Vector &mins, const Vector &maxs,
                           dplane_t *plane);
