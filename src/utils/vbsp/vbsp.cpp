...
qboolean g_bEmitSideIds = false;    // emit merged VMF side IDs in new BSP lumps
...
int g_nDXLevel =
    0; // default dxlevel if you don't specify it on the command-line.
CUtlVector<int> g_SkyAreas;
char outbase[32];

char g_szEmbedDir[MAX_PATH] = {0};

// HLTOOLS: Introduce these calcs to make the block algorithm proportional to
// the proper world coordinate extents.  Assumes square spatial constraints.
...
int block_xl = BLOCKS_MIN, block_xh = BLOCKS_MAX, block_yl = BLOCKS_MIN,
    block_yh = BLOCKS_MAX;
...
void ProcessWorldModel(void) {
  ...
  // HLTOOLS: updated to +/- MAX_COORD_INTEGER ( new world size limits /
  // worldsize.h )
  ...
  for (optimize = 0; optimize <= 1; optimize++) {
    ...
    RunThreadsOnIndividual((block_xh - block_xl + 1) *
                               (block_yh - block_yl + 1),
    ...
    if (noopt || leaked)
      break;
    if (!optimize) {
      // If we are optimizing, free the tree.  Next time we will construct it
      // again, but we'll use the information in MarkVisibleSides() so we'll
      // only split with planes that actually contribute renderable geometry
      FreeTree(tree);
    }
  }
  ...
  // This unifies the vertex list for all edges (splits collinear edges to
  // remove t-junctions) It also welds the list of vertices out of each
  // winding/portal and rounds nearly integer verts to integer
  pLeafFaceList = FixTjuncs(tree->headnode, pLeafFaceList);

  // With expanded array limits (faces, leaffaces, edges, surfedges, prims),
  // WriteBSP can run to completion even for very large maps.
  // This lets us count both vertices AND leaffaces in countverts mode.
...
void ProcessSubModel() {
  ...
  // This would wind up crashing the engine because we'd have a negative leaf
  // index in dmodel_t::headnode.
...
static tree_t *ClipOccluderBrushes() {
  ...
  bspbrush_t *list =
      MakeBspBrushList(mapBrushes.Base(), nBrushCount, mins, maxs);
...
//-----------------------------------------------------------------------------
static void GenerateOccluderSideList(int nEntity,
                                     CUtlVector<side_t *> &occluderSides) {
...
//-----------------------------------------------------------------------------
static void GenerateOccluderFaceList(node_t *pOccluderNode,
                                     CUtlVector<face_t *> &occluderFaces) {
...
static void EmitOccluderBrushes() {
  ...
  for (entity_num = 1; entity_num < num_entities; ++entity_num) {
    ...
    // Output only those parts of the occluder tree which are a part of the
    // brush
    ...
    occluderData.polycount =
        g_OccluderPolyData.Count() - occluderData.firstpoly;

    // Mark this brush as not having brush geometry so it won't be re-emitted
    // with a brush model
    {
...
// Assign occluder areas (must happen *after* the world model is processed)
//-----------------------------------------------------------------------------
void AssignAreaToOccluder(int nOccluder, tree_t *pTree,
                          bool bCrossAreaPortals) {
  ...
  for (int j = 0; j < g_OccluderData[nOccluder].polycount; ++j) {
    ...
    for (int k = 0; k < pOccluderPoly->vertexcount; ++k) {
      ...
      node_t *pNode =
          NodeForPoint(pTree->headnode, dvertexes[nVertexIndex].point);

      SetOccluderArea(nOccluder, pNode->area, nEntityNum);

      int nOtherSideIndex;
      portal_t *pPortal;
      for (pPortal = pNode->portals; pPortal;
           pPortal = pPortal->next[!nOtherSideIndex]) {
        nOtherSideIndex = (pPortal->nodes[0] == pNode) ? 1 : 0;
        if (!pPortal->onnode)
          continue; // edge of world

        // Don't cross over area portals for the area check
        if ((!bCrossAreaPortals) &&
            pPortal->nodes[nOtherSideIndex]->contents & CONTENTS_AREAPORTAL)
          continue;

        int nAdjacentArea = pPortal->nodes[nOtherSideIndex]
                                ? pPortal->nodes[nOtherSideIndex]->area
                                : 0;
...
void AssignOccluderAreas(tree_t *pTree) {
  for (int i = 0; i < g_OccluderData.Count(); ++i) {
    ...
    // This can only have happened if the only valid portal out leads into an
    // areaportal
...
void ProcessModels(void) {
  ...
  if (g_bCountVertsOnly) {
    // Output counts for machine parsing and skip BSP file write.
    // Report EXCEEDED when counts surpass the BSP format's real limits
    // (65536), not our expanded internal array capacities.
    Msg("VERTCOUNT: %d\n", numvertexes);
    Msg("LEAFFACECOUNT: %d\n", numleaffaces);
    Msg("FACECOUNT: %d\n", numfaces);
    if (numvertexes >= 65536) {
      Msg("VERTCOUNT_EXCEEDED: true\n");
    }
    if (numleaffaces >= 65536) {
      Msg("LEAFFACECOUNT_EXCEEDED: true\n");
    }
    if (numfaces >= 65536) {
      Msg("FACECOUNT_EXCEEDED: true\n");
    }
    return;
  }

  // ─── Near-limit warnings ─────────────────────────────────────────────────
  // When counts are close to the BSP format limits, the engine may crash
  // if full visibility isn't computed (i.e. vvis -fast or no vvis at all),
  // because it tries to render all faces at once without proper PVS culling.
  {
    const int BSP_LIMIT = 65536;
    const int WARN_THRESHOLD = (int)(BSP_LIMIT * 0.95); // 62259
    bool nearLimit = false;

    if (numvertexes >= WARN_THRESHOLD) {
      Warning(
          "\n*** WARNING: Vertices at %d/%d (%.1f%%) — near BSP limit! ***\n",
          numvertexes, BSP_LIMIT, 100.0 * numvertexes / BSP_LIMIT);
      nearLimit = true;
    }
    if (numleaffaces >= WARN_THRESHOLD) {
      Warning(
          "*** WARNING: Leaffaces at %d/%d (%.1f%%) — near BSP limit! ***\n",
          numleaffaces, BSP_LIMIT, 100.0 * numleaffaces / BSP_LIMIT);
      nearLimit = true;
    }
    if (numfaces >= WARN_THRESHOLD) {
      Warning("*** WARNING: Faces at %d/%d (%.1f%%) — near BSP limit! ***\n",
              numfaces, BSP_LIMIT, 100.0 * numfaces / BSP_LIMIT);
      nearLimit = true;
    }
    if (nearLimit) {
      Warning("*** Running vvis.exe -fast on this map may cause engine "
              "crashes! ***\n");
      Warning(
          "*** Use full vvis.exe (without -fast) for stable results. ***\n\n");
    }
  }
...
int RunVBSP(int argc, char **argv) {
  ...
  // Maintaining legacy behavior here to avoid breaking tools: regardless of the
  // extension we are passed, we strip it to get the "source" name, and append
  ...
  for (i = 1; i < argc; i++) {
    ...
    } else if (!Q_stricmp(argv[i], "-forcedynamicpropsasstatic")) {
      Msg("Forcing all models to be usable as static props (implies "
          "-allowdynamicpropsasstatic)\n");
      g_bForceDynamicPropsAsStatic = true;
      g_bAllowDynamicPropsAsStatic = true;
    ...
    } else if (!Q_stricmp(argv[i], "-countverts")) {
      Msg("countverts = true\n");
      g_bCountVertsOnly = true;
    } else if (!Q_stricmp(argv[i], "-emitsideids")) {
      Msg("emitsideids = true\n");
      g_bEmitSideIds = true;
  ...
  if (i != argc - 1) {
    ...
    Warning(
        ...
        "                    line options).\n"
        "\n"
        "  -onlyents   : This option causes vbsp only import the entities from "
        "the .vmf\n"
        "                file. -onlyents won't reimport brush models.\n"
        "  -onlyprops  : Only update the static props and detail props.\n"
        "  -glview     : Writes .gl files in the current directory that can be "
        "viewed\n"
        "                with glview.exe. If you use -tmpout, it will write "
        "the files\n"
        "                into the \\tmp folder.\n"
        "  -nodetail   : Get rid of all detail geometry. The geometry left "
        "over is\n"
        "                what affects visibility.\n"
        "  -nowater    : Get rid of water brushes.\n"
        "  -low        : Run as an idle-priority process.\n"
        "  -embed <directory>  : Use <directory> as an additional search path "
        "for assets\n"
        "                        and embed all assets in this directory into "
        "the compiled\n"
        "                        map\n"
        "\n"
    ...
    if (verbose) {
      Warning(
          ...
          "  -verboseentities: If -v is on, this disables verbose output for "
          "submodels.\n"
          ...
          "  -noopt       : By default, vbsp removes the 'outer shell' of the "
          "map, which\n"
          "                 are all the faces you can't see because you can "
          "never get\n"
          ...
          "  -micro <#>   : vbsp will warn when brushes are output with a "
          "volume less\n"
          ...
          "  -block # #      : Control the grid size mins that vbsp chops the "
          "level on.\n"
          "  -blocks # # # # : Enter the mins and maxs for the grid size vbsp "
          "uses.\n"
          "  -dumpstaticprops: Dump static props to staticprop*.txt\n"
          "  -allowdynamicpropsasstatic: Allow all models with the 'static' "
          "flag in the\n"
          "                              model viewer to be used on "
          "prop_static, even when\n"
          "                              their propdata doesn't contain "
          "'allowstatic'.\n"
          "  -forcedynamicpropsasstatic : Force ALL models onto prop_static,"
          "even those not compiled with\n"
          "                               $staticprop. Collision and rendering "
          "may be incorrect for models\n"
          "                               with multiple bones.\n"
          "                               Implies -allowdynamicpropsasstatic.\n"
          "  -dumpcollide    : Write files with collision info.\n"
          "  -forceskyvis	   : Enable vis calculations in 3d skybox "
          "leaves\n"
          "  -luxelscale #   : Scale all lightmaps by this amount (default: "
          "1.0).\n"
          "  -minluxelscale #: No luxel scale will be lower than this amount "
          "(default: 1.0).\n"
          "  -lightifmissing : Force lightmaps to be generated for all "
          "surfaces even if\n"
          "                    they don't need lightmaps.\n"
          "  -keepstalezip   : Keep the BSP's zip files intact but regenerate "
          "everything\n"
          "                    else.\n"
          "  -virtualdispphysics : Use virtual (not precomputed) displacement "
          "collision models\n"
          "  -xbox           : Enable mandatory xbox options\n"
          "  -x360		   : Generate Xbox360 version of vsp\n"
          "  -nox360		   : Disable generation Xbox360 version of vsp "
          "(default)\n"
          "  -replacematerials : Substitute materials according to "
          "materialsub.txt in content\\maps\n"
          "  -FullMinidumps  : Write large minidumps on crash.\n"
          "  -countverts     : Count vertices only, do not write BSP file. "
          "Outputs\n"
          "                    'VERTCOUNT: N' for machine parsing.\n");
    }
  ...
  if (onlyents) {
    LoadBSPFile(mapFile);
    num_entities = 0;
    // Clear out the cubemap samples since they will be reparsed even with
    // -onlyents
    g_nCubemapSamples = 0;

    // Mark as stale since the lighting could be screwed with new ents.
    AddBufferToPak(GetPakFile(), "stale.txt", "stale", strlen("stale") + 1,
                   false);
    ...
    // Make sure that we have a water lod control eneity if we have water in the
    // map.
