...
//-----------------------------------------------------------------------------
// Compute the centroid of a BSP face from dfaces/dsurfedges/dedges/dvertexes.
//-----------------------------------------------------------------------------
static Vector ComputeFaceCentroid(int faceIndex) {
  dface_t *pFace = &dfaces[faceIndex];
  Vector centroid(0, 0, 0);
  int numVerts = 0;

  for (int i = 0; i < pFace->numedges; i++) {
    int edgeIndex = dsurfedges[pFace->firstedge + i];
    int vertIndex;
    if (edgeIndex >= 0)
      vertIndex = dedges[edgeIndex].v[0];
    else
      vertIndex = dedges[-edgeIndex].v[1];

    centroid += dvertexes[vertIndex].point;
    numVerts++;
  }
...
//-----------------------------------------------------------------------------
// Sort helper: keep the faces closest to the overlay origin.
//-----------------------------------------------------------------------------
struct FaceDistEntry {
  int faceIndex;
  float distSq;
};

static int FaceDistCompare(const void *a, const void *b) {
  float da = ((const FaceDistEntry *)a)->distSq;
  float db = ((const FaceDistEntry *)b)->distSq;
  if (da < db)
    return -1;
  if (da > db)
    return 1;
...
//-----------------------------------------------------------------------------
// Distance-sorted overlay face truncation.
// When an overlay has more faces than the engine limit, keep the closest N.
//-----------------------------------------------------------------------------
static void TruncateFaceListByDistance(CUtlVector<int> &faceList,
                                       mapoverlay_t *pOverlay, int maxFaces) {
  int nFaceCount = faceList.Count();
  if (nFaceCount <= maxFaces)
    return;

  FaceDistEntry *entries =
      (FaceDistEntry *)_alloca(nFaceCount * sizeof(FaceDistEntry));
  for (int i = 0; i < nFaceCount; i++) {
    entries[i].faceIndex = faceList[i];
    Vector centroid = ComputeFaceCentroid(faceList[i]);
    entries[i].distSq = (centroid - pOverlay->vecOrigin).LengthSqr();
  }

  qsort(entries, nFaceCount, sizeof(FaceDistEntry), FaceDistCompare);

  faceList.RemoveAll();
  for (int i = 0; i < maxFaces; i++) {
    faceList.AddToTail(entries[i].faceIndex);
  }

  Warning("overlay %s at (%.1f %.1f %.1f): %d faces, keeping closest %d\n",
          pOverlay->szMaterialName, pOverlay->vecOrigin.x,
          pOverlay->vecOrigin.y, pOverlay->vecOrigin.z, nFaceCount, maxFaces);
}
...
int Overlay_GetFromEntity(entity_t *pMapEnt) {
  ...
  if (pMapOverlay->m_nRenderOrder < 0 ||
    ...
    Error("Overlay (%s) at %f %f %f has invalid render order (%d).\n",
          ValueForKey(pMapEnt, "material"), pMapOverlay->vecOrigin.x,
          pMapOverlay->vecOrigin.y, pMapOverlay->vecOrigin.z,
...
void Overlay_UpdateSideLists(int StartIndex) {
  ...
  for (int iMapOverlay = StartIndex; iMapOverlay < nMapOverlayCount;
       ++iMapOverlay) {
...
void Overlay_EmitOverlayFace(mapoverlay_t *pMapOverlay) {
  ...
  if (pOverlay) {
    ...
    // Face List — keep closest faces when over the engine limit
    TruncateFaceListByDistance(pMapOverlay->aFaceList, pMapOverlay,
...
// Output : none
//-----------------------------------------------------------------------------
void Overlay_Translate(mapoverlay_t *pOverlay, Vector &OriginOffset,
                       QAngle &AngleOffset, matrix3x4_t &Matrix) {
  ...
  if (!tmpMatrix.IsIdentity()) {
    ...
    bool bIsUnit =
        (fequal(fScaleU, 1.0f, 0.0001) && fequal(fScaleV, 1.0f, 0.0001) &&
         fequal(flScaleNormal, 1.0f, 0.0001));
    bool bIsPerp = (fequal(DotProduct(vecU, vecV), 0.0f, 0.0025) &&
                    fequal(DotProduct(vecU, vecNormal), 0.0f, 0.0025) &&
                    fequal(DotProduct(vecV, vecNormal), 0.0f, 0.0025));

    //		if ( fequal(fScaleU,1,0.0001) && fequal(fScaleV,1,0.0001) &&
    // fequal(DotProduct( vecU, vecV ),0,0.0025) )
    if (bIsUnit && bIsPerp) {
      ...
      for (int iHandle = 0; iHandle < OVERLAY_HANDLES_COUNT; iHandle++) {
        ...
        Vector vecPos = (vecUV.x * pOverlay->vecBasis[OVERLAY_BASIS_U] +
                         vecUV.y * pOverlay->vecBasis[OVERLAY_BASIS_V]);
