...
DEFINE_FIELD(ident, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_INTEGER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(CompressedLightCube) // array of 6 ColorRGBExp32
                                                     // (3 bytes and 1 char)
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(
            dleafambientlighting_t) // array of 6 ColorRGBExp32 (3 bytes and 1
                                    // char)
...
static const char *s_LumpNames[] = {
    ...
    "LUMP_AREAS",                          // 20
    "LUMP_AREAPORTALS",                    // 21
    "LUMP_FACE_SIDEIDS_INDEX",             // 22
    "LUMP_FACE_SIDEIDS_DATA",              // 23
    "LUMP_UNUSED2",                        // 24
    "LUMP_UNUSED3",                        // 25
    "LUMP_DISPINFO",                       // 26
...
// "-hdr" tells us to use the HDR fields (if present) on the light sources.
// Also, tells us to write out the HDR lumps for lightmaps, ambient leaves, and
// lights sources.
bool g_bHDR = false;
...
int g_numvertnormalindices; // dfaces reference these. These index
                            // g_vertnormals.
...
// Side-ID metadata: per-face index + packed side ID data
// Only populated when -emitsideids is used in VBSP
CUtlVector<dfacesideid_index_t> g_FaceSideIdIndex;
CUtlVector<int> g_FaceSideIdData;
...
//-----------------------------------------------------------------------------
void ClearPakFile(IZip *pak) { pak->Reset(); }
...
//			length -
//-----------------------------------------------------------------------------
void AddBufferToPak(IZip *pak, const char *pRelativeName, void *data,
...
void AddDirToPak(IZip *pak, const char *pDirPath, const char *pPakPrefix) {
  ...
  const char *szFindResult =
      g_pFullFileSystem->FindFirst(szEnumerateDir, &handle);
  do {
    if (szFindResult[0] != '.') {
      char szPakName[MAX_PATH] = {0};
      char szFullPath[MAX_PATH] = {0};
      if (pPakPrefix) {
        V_snprintf(szPakName, sizeof(szPakName), "%s/%s", pPakPrefix,
                   szFindResult);
      } else {
        V_strncpy(szPakName, szFindResult, sizeof(szPakName));
      }
      V_snprintf(szFullPath, sizeof(szFullPath), "%s/%s", pDirPath,
                 szFindResult);
...
GameLumpHandle_t CGameLump::NextGameLump(GameLumpHandle_t handle) {
  ...
  return (m_GameLumps.IsValidIndex(handle)) ? m_GameLumps.Next(handle)
                                            : InvalidGameLump();
}

GameLumpHandle_t CGameLump::InvalidGameLump() { return 0xFFFF; }

//-----------------------------------------------------------------------------
// Game lump creation/destruction method
//-----------------------------------------------------------------------------

GameLumpHandle_t CGameLump::CreateGameLump(GameLumpId_t id, int size, int flags,
                                           int version) {
...
void CGameLump::DestroyAllGameLumps() { m_GameLumps.RemoveAll(); }
...
void CGameLump::SwapGameLump(GameLumpId_t id, int version, byte *dest,
                             byte *src, int length) {
  int count = 0;
  switch (id) {
  case GAMELUMP_STATIC_PROPS:
    ...
    g_Swap.SwapFieldsToTargetEndian((StaticPropDictLump_t *)dest,
                                    (StaticPropDictLump_t *)src, count);
    ...
    g_Swap.SwapFieldsToTargetEndian((StaticPropLeafLump_t *)dest,
                                    (StaticPropLeafLump_t *)src, count);
  ...
  case GAMELUMP_DETAIL_PROPS:
    ...
    g_Swap.SwapFieldsToTargetEndian((DetailObjectDictLump_t *)dest,
                                    (DetailObjectDictLump_t *)src, count);
  ...
  case GAMELUMP_DETAIL_PROP_LIGHTING:
    ...
    g_Swap.SwapFieldsToTargetEndian((DetailPropLightstylesLump_t *)dest,
                                    (DetailPropLightstylesLump_t *)src, count);
    src += sizeof(DetailObjectDictLump_t) * count;
    dest += sizeof(DetailObjectDictLump_t) * count;
    break;
...
void CGameLump::ParseGameLump(dheader_t *pHeader) {
  ...
  if (length > 0) {
    ...
    dgamelumpheader_t *pGameLumpHeader =
        (dgamelumpheader_t *)((byte *)pHeader + ofs);
    if (g_bSwapOnLoad) {
    ...
    for (int i = 0; i < pGameLumpHeader->lumpCount; ++i) {
      ...
      GameLumpHandle_t lump = g_GameLumps.CreateGameLump(
          pGameLump[i].id, length, pGameLump[i].flags, pGameLump[i].version);
      if (g_bSwapOnLoad) {
        SwapGameLump(pGameLump[i].id, pGameLump[i].version,
                     (byte *)g_GameLumps.GetGameLump(lump),
                     (byte *)pHeader + pGameLump[i].fileofs, length);
      } else {
        memcpy(g_GameLumps.GetGameLump(lump),
               (byte *)pHeader + pGameLump[i].fileofs, length);
      }
    }
  }
...
static void AddGameLumps() {
  ...
  dgamelump_t dict;
  int offset =
      lump->fileofs + sizeof(header) + clumpCount * sizeof(dgamelump_t);
  GameLumpHandle_t h;
  for (h = g_GameLumps.FirstGameLump(); h != g_GameLumps.InvalidGameLump();
       h = g_GameLumps.NextGameLump(h)) {
  ...
  for (h = g_GameLumps.FirstGameLump(); h != g_GameLumps.InvalidGameLump();
    ...
    if (g_bSwapOnWrite) {
      g_GameLumps.SwapGameLump(g_GameLumps.GetGameLumpId(h),
                               g_GameLumps.GetGameLumpVersion(h),
                               (byte *)g_GameLumps.GetGameLump(h),
                               (byte *)g_GameLumps.GetGameLump(h), lumpsize);
    }
...
static void AddOcclusionLump() {
  ...
  int nLumpLength = nOccluderCount * sizeof(doccluderdata_t) +
                    ...
                    nOccluderVertexIndices * sizeof(int) + 3 * sizeof(int);
  ...
  // Data is swapped in place, so the 'Count' variables aren't safe to use after
  // they're written
  ...
  WriteData((doccluderpolydata_t *)g_OccluderPolyData.Base(),
            g_OccluderPolyData.Count());
  WriteData(FIELD_INTEGER, &nOccluderVertexIndices);
  WriteData(FIELD_INTEGER, (int *)g_OccluderVertexIndices.Base(),
            g_OccluderVertexIndices.Count());
}
...
static void UnserializeOcclusionLumpV2(CUtlBuffer &buf) {
  ...
  if (nCount) {
    if (g_bSwapOnLoad) {
      g_Swap.SwapBufferToTargetEndian((int *)buf.PeekGet(),
                                      (int *)buf.PeekGet(), nCount);
    }
    g_OccluderVertexIndices.SetCount(nCount);
    buf.Get(g_OccluderVertexIndices.Base(),
            nCount * sizeof(g_OccluderVertexIndices[0]));
  }
}
...
DEFINE_FIELD(size, FIELD_INTEGER), DEFINE_FIELD(vphysicsID, FIELD_INTEGER),
    DEFINE_FIELD(version, FIELD_SHORT), DEFINE_FIELD(modelType, FIELD_SHORT),
    END_BYTESWAP_DATADESC()
  ...
  do {
    if (g_bSwapOnLoad) {
      g_Swap.SwapFieldsToTargetEndian((dphysmodel_t *)pSrcAligned,
                                      (dphysmodel_t *)pSrc);
    } else {
  ...
  do {
    ...
    if (g_bSwapOnWrite) {
      g_Swap.SwapFieldsToTargetEndian((dphysmodel_t *)pDest,
                                      (dphysmodel_t *)pSrc);
    } else {
    ...
    if (pPhysModel->dataSize > 0) {
      ...
      if (g_bSwapOnWrite) {
        ...
        physcollision->VCollideLoad(&collide, pPhysModel->solidCount,
                                    (const char *)pSrc, dataSize, false);
      }
      ...
      for (int i = 0; i < pPhysModel->solidCount; ++i) {
        ...
        // HACK: Need either surfaceSize or moppSize - both sit at the same
      ...
      for (int i = 0; i < pPhysModel->solidCount; ++i) {
        if (collide.solids[i]) {
          ...
          pSrc += sizeof(int);
          pDest += sizeof(int);
          int offset = physcollision->CollideWrite(
              (char *)pDest, collide.solids[i], g_bSwapOnWrite);
          pSrc += offset;
          pDest += offset;
        } else {
...
static void SwapPhysdispLump(byte *pDest, byte *pSrc, int count) {
  // the format of this lump is one unsigned short dispCount, then dispCount
  // unsigned shorts of sizes followed by an array of variable length (each
  // element is the length of the corresponding entry in the previous table)
  // byte-stream data structure of the displacement collision models these
  // byte-stream structs are endian-neutral because each element is byte-sized
  unsigned short dispCount = *(unsigned short *)pSrc;
  if (g_bSwapOnLoad) {
    g_Swap.SwapBufferToTargetEndian(&dispCount);
  }
  g_Swap.SwapBufferToTargetEndian((unsigned short *)pDest,
                                  (unsigned short *)pSrc, dispCount + 1);
...
static void SwapVisibilityLump(byte *pDest, byte *pSrc, int count) {
  ...
  g_Swap.SwapBufferToTargetEndian(pDest + hdrSize, pSrc + hdrSize,
                                  count - hdrSize);
}

//=============================================================================
void Lumps_Init(void) { memset(&g_Lumps, 0, sizeof(g_Lumps)); }

int LumpVersion(int lump) { return g_pBSPHeader->lumps[lump].version; }

bool HasLump(int lump) { return g_pBSPHeader->lumps[lump].filelen > 0; }
...
int CopyLumpInternal(int fieldType, int lump, T *dest, int forceVersion) {
  ...
  if (g_bSwapOnLoad) {
    ...
    default:
      g_Swap.SwapBufferToTargetEndian(dest, (T *)((byte *)g_pBSPHeader + ofs),
                                      count);
      break;
    }
  } else {
...
template <class T>
void CopyLump(int fieldType, int lump, CUtlVector<T> &dest,
              int forceVersion = -1) {
...
template <class T>
void CopyOptionalLump(int fieldType, int lump, CUtlVector<T> &dest,
                      int forceVersion = -1) {
...
template <class T>
int CopyVariableLump(int fieldType, int lump, void **dest,
                     int forceVersion = -1) {
...
//-----------------------------------------------------------------------------
template <class T> int CopyLumpInternal(int lump, T *dest, int forceVersion) {
  g_Lumps.bLumpParsed[lump] = true;
  ...
  if (g_bSwapOnLoad) {
    g_Swap.SwapFieldsToTargetEndian(dest, (T *)((byte *)g_pBSPHeader + ofs),
                                    count);
  } else {
    memcpy(dest, (byte *)g_pBSPHeader + ofs, length);
  }
...
template <class T> int CopyLump(int lump, T *dest, int forceVersion = -1) {
...
void Lumps_Parse(void) {
  ...
  for (i = 0; i < HEADER_LUMPS; i++) {
    if (!g_Lumps.bLumpParsed[i] && g_pBSPHeader->lumps[i].filelen) {
      g_Lumps.size[i] =
          CopyVariableLump<byte>(FIELD_CHARACTER, i, &g_Lumps.pLumps[i], -1);
...
int LoadLeafs(void) {
  ...
  case 0: {
    ...
    void *pSrcBase =
        ((byte *)g_pBSPHeader + g_pBSPHeader->lumps[LUMP_LEAFS].fileofs);
    ...
    dleafambientlighting_t *pDstLeafAmbientLighting =
        &g_LeafAmbientLightingLDR[0];
    for (int i = 0; i < count; i++) {
      ...
      *pDst = *((dleaf_t *)(void *)pSrc);
      ...
      pDstLeafAmbientLighting->x = pDstLeafAmbientLighting->y =
          pDstLeafAmbientLighting->z = pDstLeafAmbientLighting->pad = 0;
      pDst++;
      pSrc++;
...
void LoadLeafAmbientLighting(int numLeafs) {
  ...
  // old BSP with ambient, or new BSP with no lighting, convert ambient light to
  // new format or create dummy ambient
  if (!HasLump(LUMP_LEAF_AMBIENT_INDEX)) {
    ...
    if (HasLump(LUMP_LEAF_AMBIENT_LIGHTING)) {
      Assert(LumpVersion(LUMP_LEAF_AMBIENT_LIGHTING) !=
             LUMP_LEAF_AMBIENT_LIGHTING_VERSION);
    }
    if (HasLump(LUMP_LEAF_AMBIENT_LIGHTING_HDR)) {
      Assert(LumpVersion(LUMP_LEAF_AMBIENT_LIGHTING_HDR) !=
             LUMP_LEAF_AMBIENT_LIGHTING_VERSION);
    }

    void *pSrcBase = ((byte *)g_pBSPHeader +
                      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING].fileofs);
    ...
    void *pSrcBaseHDR =
        ((byte *)g_pBSPHeader +
         g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING_HDR].fileofs);
    ...
    for (int i = 0; i < numLeafs; i++) {
      ...
      Q_memset(&g_LeafAmbientLightingLDR[i], 0,
               sizeof(g_LeafAmbientLightingLDR[i]));
      Q_memset(&g_LeafAmbientLightingHDR[i], 0,
               sizeof(g_LeafAmbientLightingHDR[i]));
...
//-----------------------------------------------------------------------------
//	Low level BSP opener for external parsing. Parses headers, but nothing
// else. 	You must close the BSP, via CloseBSPFile().
//-----------------------------------------------------------------------------
...
void LoadBSPFile(const char *filename) {
  ...
  CopyLump(FIELD_CHARACTER, LUMP_DISP_LIGHTMAP_SAMPLE_POSITIONS,
  ...
  CopyOptionalLump(FIELD_CHARACTER, LUMP_FACE_SIDEIDS_INDEX, g_FaceSideIdIndex);
  CopyOptionalLump(FIELD_INTEGER, LUMP_FACE_SIDEIDS_DATA, g_FaceSideIdData);
  ...
  CopyOptionalLump(FIELD_CHARACTER, LUMP_LIGHTING, dlightdataLDR,
                   LUMP_LIGHTING_VERSION);
  CopyOptionalLump(FIELD_CHARACTER, LUMP_LIGHTING_HDR, dlightdataHDR,
                   LUMP_LIGHTING_VERSION);
  ...
  g_PhysCollideSize = CopyVariableLump<byte>(FIELD_CHARACTER, LUMP_PHYSCOLLIDE,
                                             (void **)&g_pPhysCollide);
  g_PhysDispSize = CopyVariableLump<byte>(FIELD_CHARACTER, LUMP_PHYSDISP,
                                          (void **)&g_pPhysDisp);

  g_numvertnormals =
      CopyLump(FIELD_VECTOR, LUMP_VERTNORMALS, (float *)g_vertnormals);
  g_numvertnormalindices =
      CopyLump(FIELD_SHORT, LUMP_VERTNORMALINDICES, g_vertnormalindices);

  g_nClipPortalVerts =
      CopyLump(FIELD_VECTOR, LUMP_CLIPPORTALVERTS, (float *)g_ClipPortalVerts);
  ...
  // It's assumed by other code that the data lump is filled with C strings. We
  // need to make sure that the buffer as a whole ends with a '\0'.
  ...
  */
  ...
  byte *pakbuffer = NULL;
  int paksize = CopyVariableLump<byte>(FIELD_CHARACTER, LUMP_PAKFILE,
                                       (void **)&pakbuffer);
  if (paksize > 0) {
    GetPakFile()->ActivateByteSwapping(IsX360());
  ...
  // NOTE: Do NOT call CopyLump after Lumps_Parse() it parses all un-Copied
  // lumps parse any additional lumps
  Lumps_Parse();
...
void UnloadBSPFile() {
  ...
  dfaceids.Purge();
  g_FaceSideIdIndex.Purge();
  g_FaceSideIdData.Purge();
...
template <class T> static void SwapInPlace(T *pData, int count) {
  if (!pData)
    return;
...
template <class T> static void SwapInPlace(int fieldType, T *pData, int count) {
  if (!pData)
    return;
...
//-----------------------------------------------------------------------------
template <class T> static void WriteData(int fieldType, T *pData, int count) {
  if (g_bSwapOnWrite) {
...
template <class T> static void WriteData(T *pData, int count) {
  if (g_bSwapOnWrite) {
...
void WriteBSPFile(const char *filename, char *pUnused) {
  ...
  AddLump(LUMP_LEAF_AMBIENT_LIGHTING, g_LeafAmbientLightingLDR,
          LUMP_LEAF_AMBIENT_LIGHTING_VERSION);
  AddLump(LUMP_LEAF_AMBIENT_INDEX, g_LeafAmbientIndexLDR);
  AddLump(LUMP_LEAF_AMBIENT_INDEX_HDR, g_LeafAmbientIndexHDR);
  AddLump(LUMP_LEAF_AMBIENT_LIGHTING_HDR, g_LeafAmbientLightingHDR,
          LUMP_LEAF_AMBIENT_LIGHTING_VERSION);
  ...
  // Emit side-ID metadata lumps (populated by -emitsideids)
  if (g_FaceSideIdIndex.Count() > 0) {
    AddLumpInternal(LUMP_FACE_SIDEIDS_INDEX, g_FaceSideIdIndex.Base(),
                    g_FaceSideIdIndex.Count() * sizeof(dfacesideid_index_t), 0);
    AddLumpInternal(LUMP_FACE_SIDEIDS_DATA, g_FaceSideIdData.Base(),
                    g_FaceSideIdData.Count() * sizeof(int), 0);
  }
  ...
  AddLump(LUMP_CLIPPORTALVERTS, (float *)g_ClipPortalVerts,
          g_nClipPortalVerts * 3);
...
bool GenerateNextLumpFileName(const char *bspfilename, char *lumpfilename,
                              int buffsize) {
...
void WriteLumpToFile(char *filename, int lump) {
  ...
  lumpHeader.lumpOffset =
      sizeof(lumpfileheader_t); // Lump starts after the header
...
int ArrayUsage(const char *szItem, int items, int maxitems, int itemsize) {
  ...
  Msg("%-17.17s %8i/%-8i %8i/%-8i (%4.1f%%) ", szItem, items, maxitems,
...
int GlobUsage(const char *szItem, int itemstorage, int maxstorage) {
  ...
  Msg("%-17.17s     [variable]    %8i/%-8i (%4.1f%%) ", szItem, itemstorage,
...
void PrintBSPFileSizes(void) {
  ...
  Msg("\n");
  Msg("%-17s %16s %16s %9s \n", "Object names", "Objects/Maxobjs",
      "Memory / Maxmem", "Fullness");
  Msg("%-17s %16s %16s %9s \n", "------------", "---------------",
      "---------------", "--------");

  totalmemory +=
      ArrayUsage("models", nummodels, ENTRIES(dmodels), ENTRYSIZE(dmodels));
  totalmemory +=
      ArrayUsage("brushes", numbrushes, ENTRIES(dbrushes), ENTRYSIZE(dbrushes));
  totalmemory += ArrayUsage("brushsides", numbrushsides, ENTRIES(dbrushsides),
                            ENTRYSIZE(dbrushsides));
  totalmemory +=
      ArrayUsage("planes", numplanes, ENTRIES(dplanes), ENTRYSIZE(dplanes));
  totalmemory += ArrayUsage("vertexes", numvertexes, ENTRIES(dvertexes),
                            ENTRYSIZE(dvertexes));
  totalmemory +=
      ArrayUsage("nodes", numnodes, ENTRIES(dnodes), ENTRYSIZE(dnodes));
  totalmemory += ArrayUsage("texinfos", texinfo.Count(), MAX_MAP_TEXINFO,
                            sizeof(texinfo_t));
  totalmemory +=
      ArrayUsage("texdata", numtexdata, ENTRIES(dtexdata), ENTRYSIZE(dtexdata));

  totalmemory +=
      ArrayUsage("dispinfos", g_dispinfo.Count(), 0, sizeof(ddispinfo_t));
  totalmemory +=
      ArrayUsage("disp_verts", g_DispVerts.Count(), 0, sizeof(g_DispVerts[0]));
  totalmemory +=
      ArrayUsage("disp_tris", g_DispTris.Count(), 0, sizeof(g_DispTris[0]));
  totalmemory +=
      ArrayUsage("disp_lmsamples", g_DispLightmapSamplePositions.Count(), 0,
                 sizeof(g_DispLightmapSamplePositions[0]));

  totalmemory +=
      ArrayUsage("faces", numfaces, ENTRIES(dfaces), ENTRYSIZE(dfaces));
  totalmemory += ArrayUsage("hdr faces", numfaces_hdr, ENTRIES(dfaces_hdr),
                            ENTRYSIZE(dfaces_hdr));
  totalmemory += ArrayUsage("origfaces", numorigfaces, ENTRIES(dorigfaces),
                            ENTRYSIZE(dorigfaces)); // original faces
  totalmemory +=
      ArrayUsage("leaves", numleafs, ENTRIES(dleafs), ENTRYSIZE(dleafs));
  totalmemory += ArrayUsage("leaffaces", numleaffaces, ENTRIES(dleaffaces),
                            ENTRYSIZE(dleaffaces));
  totalmemory += ArrayUsage("leafbrushes", numleafbrushes,
                            ENTRIES(dleafbrushes), ENTRYSIZE(dleafbrushes));
  totalmemory +=
      ArrayUsage("areas", numareas, ENTRIES(dareas), ENTRYSIZE(dareas));
  totalmemory += ArrayUsage("surfedges", numsurfedges, ENTRIES(dsurfedges),
                            ENTRYSIZE(dsurfedges));
  totalmemory +=
      ArrayUsage("edges", numedges, ENTRIES(dedges), ENTRYSIZE(dedges));
  totalmemory +=
      ArrayUsage("LDR worldlights", numworldlightsLDR, ENTRIES(dworldlightsLDR),
                 ENTRYSIZE(dworldlightsLDR));
  totalmemory +=
      ArrayUsage("HDR worldlights", numworldlightsHDR, ENTRIES(dworldlightsHDR),
                 ENTRYSIZE(dworldlightsHDR));

  totalmemory += ArrayUsage("leafwaterdata", numleafwaterdata,
                            ENTRIES(dleafwaterdata), ENTRYSIZE(dleafwaterdata));
  totalmemory += ArrayUsage("waterstrips", g_numprimitives,
                            ENTRIES(g_primitives), ENTRYSIZE(g_primitives));
  totalmemory += ArrayUsage("waterverts", g_numprimverts, ENTRIES(g_primverts),
                            ENTRYSIZE(g_primverts));
  totalmemory += ArrayUsage("waterindices", g_numprimindices,
                            ENTRIES(g_primindices), ENTRYSIZE(g_primindices));
  totalmemory +=
      ArrayUsage("cubemapsamples", g_nCubemapSamples, ENTRIES(g_CubemapSamples),
                 ENTRYSIZE(g_CubemapSamples));
  totalmemory += ArrayUsage("overlays", g_nOverlayCount, ENTRIES(g_Overlays),
                            ENTRYSIZE(g_Overlays));
  ...
  totalmemory +=
      GlobUsage("entdata", dentdata.Count(), 384 * 1024); // goal is <384K

  totalmemory += ArrayUsage("LDR ambient table", g_LeafAmbientIndexLDR.Count(),
                            MAX_MAP_LEAFS, sizeof(g_LeafAmbientIndexLDR[0]));
  totalmemory += ArrayUsage("HDR ambient table", g_LeafAmbientIndexHDR.Count(),
                            MAX_MAP_LEAFS, sizeof(g_LeafAmbientIndexHDR[0]));
  totalmemory +=
      ArrayUsage("LDR leaf ambient lighting", g_LeafAmbientLightingLDR.Count(),
                 MAX_MAP_LEAFS, sizeof(g_LeafAmbientLightingLDR[0]));
  totalmemory +=
      ArrayUsage("HDR leaf ambient lighting", g_LeafAmbientLightingHDR.Count(),
                 MAX_MAP_LEAFS, sizeof(g_LeafAmbientLightingHDR[0]));

  totalmemory += ArrayUsage("occluders", g_OccluderData.Count(), 0,
                            sizeof(g_OccluderData[0]));
  totalmemory += ArrayUsage("occluder polygons", g_OccluderPolyData.Count(), 0,
                            sizeof(g_OccluderPolyData[0]));
  totalmemory +=
      ArrayUsage("occluder vert ind", g_OccluderVertexIndices.Count(), 0,
                 sizeof(g_OccluderVertexIndices[0]));
  ...
  if (h != g_GameLumps.InvalidGameLump())
    totalmemory +=
        GlobUsage("HDR dtl prp lght", 1, g_GameLumps.GameLumpSize(h));
...
=============
*/
void PrintBSPPackDirectory(void) { GetPakFile()->PrintDirectory(); }
...
void CalcFaceExtents(dface_t *s, int lightmapTextureMinsInLuxels[2],
  ...
  int nMaxLightmapDim = (s->dispinfo == -1)
                            ? MAX_LIGHTMAP_DIM_WITHOUT_BORDER
                            : MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER;
  for (i = 0; i < 2; i++) {
    ...
    if (lightmapTextureSizeInLuxels[i] > nMaxLightmapDim + 1) {
      ...
      Error("Bad surface extents - surface is too big to have a "
            "lightmap\n\tmaterial %s around point (%.1f %.1f "
            "%.1f)\n\t(dimension: %d, %d>%d)\n",
            TexDataStringTable_GetString(
                dtexdata[texinfo[s->texinfo].texdata].nameStringTableID),
            point.x, point.y, point.z, (int)i,
            (int)lightmapTextureSizeInLuxels[i], (int)(nMaxLightmapDim + 1));
    }
  }
}
...
void UpdateAllFaceLightmapExtents() {
  for (int i = 0; i < numfaces; i++) {
    ...
    CalcFaceExtents(pFace, pFace->m_LightmapTextureMinsInLuxels,
                    pFace->m_LightmapTextureSizeInLuxels);
  }
}
...
public:
  ...
  bool EnumerateLeavesAtPoint(Vector const &pt, ISpatialLeafEnumerator *pEnum,
                              intp context);
  bool EnumerateLeavesInBox(Vector const &mins, Vector const &maxs,
                            ISpatialLeafEnumerator *pEnum, intp context);
  bool EnumerateLeavesInSphere(Vector const &center, float radius,
                               ISpatialLeafEnumerator *pEnum, intp context);
  bool EnumerateLeavesAlongRay(Ray_t const &ray, ISpatialLeafEnumerator *pEnum,
                               intp context);
};
...
int CToolBSPTree::LeafCount() const { return numleafs; }
...
static bool EnumerateLeavesInBox_R(int node, Vector const &mins,
  ...
  while (node >= 0) {
    ...
    if ((DotProduct(pPlane->normal, cornermax) - pPlane->dist) <=
      ...
      if (!EnumerateLeavesInBox_R(pNode->children[0], mins, maxs, pEnum,
                                  context)) {
        return false;
      }

      return EnumerateLeavesInBox_R(pNode->children[1], mins, maxs, pEnum,
                                    context);
    }
  }
...
static bool EnumerateLeavesInSphere_R(int node, Vector const &origin,
  ...
  while (node >= 0) {
    ...
    if (DotProduct(pPlane->normal, origin) + radius - pPlane->dist <=
        -TEST_EPSILON) {
      node = pNode->children[1];
    } else if (DotProduct(pPlane->normal, origin) - radius - pPlane->dist >=
               TEST_EPSILON) {
      node = pNode->children[0];
    } else {
      if (!EnumerateLeavesInSphere_R(pNode->children[0], origin, radius, pEnum,
                                     context)) {
        return false;
      }

      return EnumerateLeavesInSphere_R(pNode->children[1], origin, radius,
                                       pEnum, context);
    }
  }
...
bool CToolBSPTree::EnumerateLeavesInSphere(Vector const &center, float radius,
                                           ISpatialLeafEnumerator *pEnum,
                                           intp context) {
  ...
  while (node >= 0) {
    ...
    if (front <= -TEST_EPSILON && back <= -TEST_EPSILON) {
      ...
      if (denom == 0.0f) {
        ...
        splitfrac =
            (pPlane->dist - DotProduct(ray.m_Start, pPlane->normal)) / denom;
        if (splitfrac < 0)
          splitfrac = 0;
        else if (splitfrac > 1)
          splitfrac = 1;
      ...
      bool r = EnumerateLeavesAlongRay_R(pNode->children[side], ray, start,
                                         split, pEnum, context);
      if (!r)
        return r;
      return EnumerateLeavesAlongRay_R(pNode->children[!side], ray, split, end,
                                       pEnum, context);
    }
  }
...
bool CToolBSPTree::EnumerateLeavesAlongRay(Ray_t const &ray,
                                           ISpatialLeafEnumerator *pEnum,
                                           intp context) {
  if (!ray.m_IsSwept) {
    Vector mins, maxs;
...
static bool EnumerateNodesAlongRay_R(int node, Ray_t const &ray, float start,
  ...
  while (node >= 0) {
    ...
    if (front <= -TEST_EPSILON && back <= -TEST_EPSILON) {
      ...
      bool r = EnumerateNodesAlongRay_R(pNode->children[side], ray, start,
                                        splitfrac, pEnum, context);
      if (!r)
        return r;
      ...
      return EnumerateNodesAlongRay_R(pNode->children[!side], ray, splitfrac,
                                      end, pEnum, context);
    }
  }
...
void BuildClusterTable(void) {
  ...
  for (i = 0; i < dvis->numclusters; i++) {
    ...
    if (leafCount) {
      ...
      memcpy(g_ClusterLeaves[i].leafs.Base(), leafList,
             sizeof(int) * leafCount);
    }
  }
}

// There's a version of this in checksum_engine.cpp!!! Make sure that they
// match.
...
bool SwapVHV(void *pDestBase, void *pSrcBase) {
  ...
  HardwareVerts::FileHeader_t *pHdr =
      (HardwareVerts::FileHeader_t *)(g_bSwapOnLoad ? pDest : pSrc);
  g_Swap.SwapFieldsToTargetEndian<HardwareVerts::FileHeader_t>(
      (HardwareVerts::FileHeader_t *)pDest,
      (HardwareVerts::FileHeader_t *)pSrc);
  ...
  HardwareVerts::MeshHeader_t *pMesh =
      (HardwareVerts::MeshHeader_t *)(g_bSwapOnLoad ? pDest : pSrc);
  for (int i = 0; i < pHdr->m_nMeshes; ++i, ++pMesh, ++pSrcMesh, ++pDestMesh) {
    ...
    // (source is bgra for an Intel swap to argb. PowerPC won't swap, so we need
    // argb source.
    g_Swap.SwapBufferToTargetEndian<int>((int *)pDest, (int *)pSrc,
                                         pMesh->m_nVertexes);
  }
  return true;
}
...
void ConvertPakFileContents(const char *pInFilename) {
  ...
  while (1) {
    ...
    if (!bOK) {
      Warning(
          "Failed to load '%s' from lump pak for conversion or copy in '%s'.\n",
          relativeName, pInFilename);
      continue;
    }

    if (pExtension && !V_stricmp(pExtension, "vtf")) {
      bOK = g_pVTFConvertFunc(relativeName, sourceBuf, targetBuf,
                              g_pCompressFunc);
      if (!bOK) {
      ...
      if (g_pCompressFunc) {
        ...
        targetBuf.SeekGet(CUtlBuffer::SEEK_HEAD,
        ...
        if (bCompressed) {
          ...
          headerBuffer.Put(targetBuf.Base(),
                           sizeof(HardwareVerts::FileHeader_t));

          // reform the target with the header and then the compressed data
          targetBuf.Clear();
          targetBuf.Put(headerBuffer.Base(),
                        sizeof(HardwareVerts::FileHeader_t));
    ...
    if (!bConverted) {
      // straight copy
      AddBufferToPak(newPakFile, relativeName, sourceBuf.Base(),
                     sourceBuf.TellMaxPut(), false,
      ...
      AddBufferToPak(newPakFile, relativeName, targetBuf.Base(),
                     targetBuf.TellMaxPut(), false,
                     IZip::eCompressionType_None);
    }
...
template <class T> int SwapLumpToDisk(int fieldType, int lumpnum) {
  ...
  unsigned int fieldSize =
      (fieldType == FIELD_VECTOR) ? sizeof(Vector) : sizeof(T);
  unsigned int count = CopyLumpInternal<T>(
      fieldType, lumpnum, (T *)pBuffer, g_pBSPHeader->lumps[lumpnum].version);
  ...
  if (g_bSwapOnWrite) {
    ...
    default:
      g_Swap.SwapBufferToTargetEndian((T *)pBuffer, (T *)pBuffer,
                                      g_pBSPHeader->lumps[lumpnum].filelen /
                                          sizeof(T));
      break;
    }
  }
...
template <class T> int SwapLumpToDisk(int lumpnum) {
  ...
  // CopyLumpInternal will handle the swap on load case
  int count = CopyLumpInternal<T>(lumpnum, (T *)pBuffer,
...
void SwapLeafAmbientLightingLumpToDisk() {
  if (HasLump(LUMP_LEAF_AMBIENT_INDEX) ||
      HasLump(LUMP_LEAF_AMBIENT_INDEX_HDR)) {
    ...
    // an older leaf version would have already built the new LDR leaf
    // ambient/index
    ...
    if (HasLump(LUMP_LEAF_AMBIENT_LIGHTING_HDR)) {
      ...
      // write HDR
      if (g_bSwapOnWrite) {
        g_Swap.SwapFieldsToTargetEndian(g_LeafAmbientLightingHDR.Base(),
                                        g_LeafAmbientLightingHDR.Count());
        g_Swap.SwapFieldsToTargetEndian(g_LeafAmbientIndexHDR.Base(),
                                        g_LeafAmbientIndexHDR.Count());
      }

      SetAlignedLumpPosition(LUMP_LEAF_AMBIENT_LIGHTING_HDR);
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING_HDR].version =
          LUMP_LEAF_AMBIENT_LIGHTING_VERSION;
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING_HDR].filelen =
          g_LeafAmbientLightingHDR.Count() * sizeof(dleafambientlighting_t);
      SafeWrite(g_hBSPFile, g_LeafAmbientLightingHDR.Base(),
                g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING_HDR].filelen);

      SetAlignedLumpPosition(LUMP_LEAF_AMBIENT_INDEX_HDR);
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_INDEX_HDR].filelen =
          g_LeafAmbientIndexHDR.Count() * sizeof(dleafambientindex_t);
      SafeWrite(g_hBSPFile, g_LeafAmbientIndexHDR.Base(),
                g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_INDEX_HDR].filelen);
      ...
      if (g_bSwapOnWrite) {
        g_Swap.SwapFieldsToTargetEndian(g_LeafAmbientLightingLDR.Base(),
                                        g_LeafAmbientLightingLDR.Count());
        g_Swap.SwapFieldsToTargetEndian(g_LeafAmbientIndexLDR.Base(),
                                        g_LeafAmbientIndexLDR.Count());
      }

      SetAlignedLumpPosition(LUMP_LEAF_AMBIENT_LIGHTING);
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING].version =
          LUMP_LEAF_AMBIENT_LIGHTING_VERSION;
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING].filelen =
          g_LeafAmbientLightingLDR.Count() * sizeof(dleafambientlighting_t);
      SafeWrite(g_hBSPFile, g_LeafAmbientLightingLDR.Base(),
                g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_LIGHTING].filelen);

      SetAlignedLumpPosition(LUMP_LEAF_AMBIENT_INDEX);
      g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_INDEX].filelen =
          g_LeafAmbientIndexLDR.Count() * sizeof(dleafambientindex_t);
      SafeWrite(g_hBSPFile, g_LeafAmbientIndexLDR.Base(),
                g_pBSPHeader->lumps[LUMP_LEAF_AMBIENT_INDEX].filelen);
...
void SwapLeafLumpToDisk(void) {
  ...
  if (bOldLeafVersion) {
    ...
    // not updating the version ye, SwapLeafAmbientLightingLumpToDisk() can
    // detect
...
void BuildStaticPropNameTable() {
  ...
  if (hGameLump != g_GameLumps.InvalidGameLump()) {
    ...
    if (pGameLumpData && g_GameLumps.GameLumpSize(hGameLump)) {
      ...
      StaticPropDictLump_t *pStaticPropDictLump =
          (StaticPropDictLump_t *)pGameLumpData;
      for (int i = 0; i < count; i++) {
...
int SortLumpsByOffset(const SortedLump_t *pSortedLumpA,
                      const SortedLump_t *pSortedLumpB) {
...
bool CompressGameLump(dheader_t *pInBSPHeader, dheader_t *pOutBSPHeader,
                      CUtlBuffer &outputBuffer, CompressFunc_t pCompressFunc) {
  CByteswap byteSwap;

  dgamelumpheader_t *pInGameLumpHeader =
      (dgamelumpheader_t *)(((byte *)pInBSPHeader) +
                            pInBSPHeader->lumps[LUMP_GAME_LUMP].fileofs);
  ...
  if (IsX360()) {
    ...
    byteSwap.SwapFieldsToTargetEndian(pInGameLump,
                                      pInGameLumpHeader->lumpCount);
  }

  unsigned int newOffset = outputBuffer.TellPut();
  // Make room for gamelump header and gamelump structs, which we'll write at
  // the end
  outputBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT, sizeof(dgamelumpheader_t));
  outputBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT,
                       pInGameLumpHeader->lumpCount * sizeof(dgamelump_t));

  // Start with input lumps, and fixup
  dgamelumpheader_t sOutGameLumpHeader = *pInGameLumpHeader;
  CUtlBuffer sOutGameLumpBuf;
  sOutGameLumpBuf.Put(pInGameLump,
                      pInGameLumpHeader->lumpCount * sizeof(dgamelump_t));
  dgamelump_t *sOutGameLump = (dgamelump_t *)sOutGameLumpBuf.Base();

  // add a dummy terminal gamelump
  // purposely NOT updating the .filelen to reflect the compressed size, but
  // leaving as original size callers use the next entry offset to determine
  // compressed size
    ...
    if (pInGameLump[i].filelen) {
      if (pInGameLump[i].flags & GAMELUMPFLAG_COMPRESSED) {
        ...
        if (CLZMA::IsCompressed(pCompressedLump)) {
          ...
          unsigned int outSize = CLZMA::Uncompress(
              pCompressedLump, (unsigned char *)inputBuffer.Base());
          inputBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT, outSize);
          if (outSize != CLZMA::GetActualSize(pCompressedLump)) {
            Warning(
                "Decompressed size differs from header, BSP may be corrupt\n");
          }
        } else {
        ...
        inputBuffer.SetExternalBuffer(
            ((byte *)pInBSPHeader) + pInGameLump[i].fileofs,
            pInGameLump[i].filelen, pInGameLump[i].filelen);
      }

      bool bCompressed =
          pCompressFunc ? pCompressFunc(inputBuffer, compressedBuffer) : false;
      if (bCompressed) {
  ...
  if (IsX360()) {
    ...
    byteSwap.SwapFieldsToTargetEndian(sOutGameLump,
                                      sOutGameLumpHeader.lumpCount);
    byteSwap.SwapFieldsToTargetEndian(&sOutGameLumpHeader);
  }

  pOutBSPHeader->lumps[LUMP_GAME_LUMP].fileofs = newOffset;
  pOutBSPHeader->lumps[LUMP_GAME_LUMP].filelen =
      outputBuffer.TellPut() - newOffset;
  // We set GAMELUMPFLAG_COMPRESSED and handle compression at the sub-lump
  // level, this whole lump is not decompressable as a block.
...
bool RepackBSPCallback_LZMA(CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer) {
  ...
  unsigned char *pCompressedOutput =
      LZMA_Compress((unsigned char *)inputBuffer.Base() + inputBuffer.TellGet(),
...
bool RepackBSP(CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer,
  ...
  // This buffer grows dynamically, don't keep pointers to it around. Write out
  // header at end.
  dheader_t sOutBSPHeader = *pInBSPHeader;

  // must adhere to input lump's offset order and process according to that, NOT
  // lump num sort by offset order
  ...
  for (int i = 0; i < HEADER_LUMPS; ++i) {
    ...
    if (pSortedLump->pLump->filelen) // Otherwise its degenerate
    {
      ...
      if (pSortedLump->pLump->uncompressedSize) {
        byte *pCompressedLump =
            ((byte *)pInBSPHeader) + pSortedLump->pLump->fileofs;
        if (CLZMA::IsCompressed(pCompressedLump) &&
            pSortedLump->pLump->uncompressedSize ==
                CLZMA::GetActualSize(pCompressedLump)) {
          inputBuffer.EnsureCapacity(CLZMA::GetActualSize(pCompressedLump));
          unsigned int outSize = CLZMA::Uncompress(
              pCompressedLump, (unsigned char *)inputBuffer.Base());
          inputBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT, outSize);
      ...
      if (lumpNum == LUMP_GAME_LUMP) {
        // the game lump has to have each of its components individually
        // compressed
        CompressGameLump(pInBSPHeader, &sOutBSPHeader, outputBuffer,
                         pCompressFunc);
        ...
        while (1) {
          char relativeName[MAX_PATH];
          id = GetNextFilename(oldPakFile, id, relativeName,
                               sizeof(relativeName), fileSize);
          if (id == -1)
            break;
          ...
          bool bOK =
              ReadFileFromPak(oldPakFile, relativeName, false, sourceBuf);
          if (!bOK) {
          ...
          AddBufferToPak(newPakFile, relativeName, sourceBuf.Base(),
                         sourceBuf.TellMaxPut(), false, packfileCompression);
        ...
        sOutBSPHeader.lumps[lumpNum].filelen =
            outputBuffer.TellPut() - newOffset;
        // Note that this *lump* is uncompressed, it just contains a packfile
        // that uses compression, so we're not setting
        // lumps[lumpNum].uncompressedSize

        IZip::ReleaseZip(oldPakFile);
        IZip::ReleaseZip(newPakFile);
      } else {
        CUtlBuffer compressedBuffer;
        bool bCompressed = pCompressFunc
                               ? pCompressFunc(inputBuffer, compressedBuffer)
                               : false;
        if (bCompressed) {
...
//-----------------------------------------------------------------------------
//  For all lumps in a bsp: Loads the lump from file A, swaps it, writes it to
//  file B. This limits the memory used for the swap process which helps the
//  Xbox 360.
//
//	NOTE: These lumps will be written to the file in exactly the order they
// appear here, 	so they can be shifted around if desired for file access
// optimization.
//-----------------------------------------------------------------------------
bool SwapBSPFile(const char *pInFilename, const char *pOutFilename,
...
//-----------------------------------------------------------------------------
// Build a list of files that BSP owns, world/cubemap materials, static props,
// etc.
//-----------------------------------------------------------------------------
bool GetBSPDependants(const char *pBSPFilename, CUtlVector<CUtlString> *pList) {
  ...
  int fileId = -1;
  for (;;) {
    fileId = GetPakFile()->GetNextFilename(fileId, szFilename,
                                           sizeof(szFilename), fileSize);
    if (fileId == -1) {
      break;
    }
  ...
  for (int i = 0; i < numtexdata; i++) {
    const char *pName =
        TexDataStringTable_GetString(dtexdata[i].nameStringTableID);
  ...
  if (hGameLump != g_GameLumps.InvalidGameLump()) {
    ...
    if (pGameLumpData && g_GameLumps.GameLumpSize(hGameLump)) {
      ...
      DetailObjectDictLump_t *pDetailObjectDictLump =
          (DetailObjectDictLump_t *)pGameLumpData;
      for (int i = 0; i < count; i++) {
      ...
      if (g_GameLumps.GetGameLumpVersion(hGameLump) == 4) {
        ...
        pGameLumpData += sizeof(int);
        if (count) {
          // All detail prop sprites must lie in the material
