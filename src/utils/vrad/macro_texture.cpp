...
static CUtlDict<CMacroTextureData *, int>
    g_MacroTextureLookup; // Stores a list of unique macro textures.
static CUtlVector<CMacroTextureData *>
    g_FaceMacroTextures; // Which macro texture each face wants to use.
...
CMacroTextureData *LoadMacroTextureFile(const char *pFilename) {
  ...
  pTex->ConvertImageFormat(IMAGE_FORMAT_RGBA8888,
                           false); // Get it in a format we like.
  ...
  memcpy(pData->m_ImageData.Base(), pTex->ImageData(),
         pData->m_Width * pData->m_Height * 4);
...
void InitMacroTexture(const char *pBSPFilename) {
  // Get the world bounds (same ones used by minimaps and level designers know
  // how to use).
  int i = 0;
  ...
  Q_snprintf(vtfFilename, sizeof(vtfFilename), "materials/macro/%s/base.vtf",
             mapName);
  ...
  for (int iFace = 0; iFace < numfaces; iFace++) {
    ...
    if (iFace < g_FaceMacroTextureInfos.Count()) {
      ...
      if (stringID != 0xFFFF) {
        const char *pMacroTextureName =
            &g_TexDataStringData[g_TexDataStringTable[stringID]];
        Q_snprintf(vtfFilename, sizeof(vtfFilename), "%smaterials/%s.vtf",
                   gamedir, pMacroTextureName);
        ...
        if (!g_FaceMacroTextures[iFace]) {
          ...
          if (g_FaceMacroTextures[iFace]) {
            g_MacroTextureLookup.Insert(vtfFilename,
                                        g_FaceMacroTextures[iFace]);
          }
        }
      }
    }
...
inline Vector SampleMacroTexture(const CMacroTextureData *t,
                                 const Vector &vWorldPos) {
  int ix = (int)RemapVal(vWorldPos.x, g_MacroWorldMins.x, g_MacroWorldMaxs.x, 0,
                         t->m_Width - 0.00001);
  ...
  return Vector(pInputColor[0] / 255.0, pInputColor[1] / 255.0,
                pInputColor[2] / 255.0);
}
