...
extern IIncremental *g_pIncremental;
extern long long g_TotalRaysTraced;
...
struct directlight_t {
  ...
  // m_flStartFadeDistance and m_flEndFadeDistance, a smoothstep to zero will be
  // done, so that the light goes to zero at the end.
...
struct CPatch {
  ...
  unsigned short
      m_IterationKey; // Used to prevent touching the same patch multiple times
                      // in the same query. See IncrementPatchIterationKey().
  ...
  float
      planeDist; // Fixes up patch planes for brush models with an origin brush
  ...
  int ndxNextClusterChild; // next terminal child index in cluster
                           //	struct		patch_s		*next;
  //// next in face 	struct		patch_s		*nextparent;
  //// next in face 	struct		patch_s		*nextclusterchild;
  //// next terminal child in cluster
...
extern double g_flDirectLightingTime;
extern double g_flBounceLightingTime;
extern double g_flOtherLightingTime;
extern double g_flSceneSetupTime;
extern double g_flVisMatrixTime;
extern double g_flCSRBuildTime;
extern double g_flStartTime;
...
// Optimized variant: accepts pre-computed normalized direction and length,
// avoiding redundant direction/length computation from start/stop.
...
struct SSE_sampleLightOutput_t {
  ...
  fltx4 m_flSunAmount;
  FourVectors
      m_raySource; // Light endpoint for deferred shadow rays (GPU batching)
};

#define GATHERLFLAGS_FORCE_FAST 1
#define GATHERLFLAGS_IGNORE_NORMALS 2
#define GATHERLFLAGS_SKIP_VISIBILITY 4 // Skip TestLine - for GPU batching
...
// *dl, int facenum,
// FourVectors const& pos, FourVectors *pNormals, int
// normalCount, int iThread,
// int nLFlags = 0, 							 int
// static_prop_to_skip=-1, float flEpsilon = 0.0 ); void
// GatherSampleAmbientSkySSE( SSE_sampleLightOutput_t
// &out, directlight_t *dl, int facenum,
// FourVectors const& pos, FourVectors
//*pNormals, int normalCount, int iThread,
// int nLFlags = 0,
//// GATHERLFLAGS_xxx 						  int
/// static_prop_to_skip=-1, 						  float
/// flEpsilon = 0.0 ); void
// GatherSampleStandardLightSSE( SSE_sampleLightOutput_t &out, directlight_t
// *dl, int facenum, 						  FourVectors
// const& pos, FourVectors *pNormals, int
// normalCount, int iThread, 						  int
// nLFlags = 0, // GATHERLFLAGS_xxx int static_prop_to_skip=-1,
// float flEpsilon = 0.0 );
...
public:
  ...
  virtual bool BuildDispLuxels(lightinfo_t *pLightInfo, facelight_t *pFaceLight,
                               int ndxFace) = 0;
  ...
  virtual bool SampleRadial(int ndxFace, radial_t *pRadial, Vector const &vPos,
                            int ndxLxl, LightingValue_t *pLightSample,
                            int sampleCount, bool bPatch) = 0;
