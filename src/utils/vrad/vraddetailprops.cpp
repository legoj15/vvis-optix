...
//			*pFilename -
//-----------------------------------------------------------------------------
void DumpRayToGlView(Ray_t const &ray, float dist, Vector *pColor,
  ...
  CmdLib_FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start1.x, start1.y,
                 start1.z, pColor->x, pColor->y, pColor->z);
  vert++;
  CmdLib_FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start2.x, start2.y,
                 start2.z, pColor->x, pColor->y, pColor->z);
  vert++;
  CmdLib_FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", end.x, end.y, end.z,
...
static CUtlVector<DetailPropLightstylesLump_t> *s_pDetailPropLightStyleLump =
...
//-----------------------------------------------------------------------------
static void ComputeWorldCenter(DetailObjectLump_t &prop, Vector &center,
                               Vector &normal) {
  ...
  case DETAIL_PROP_TYPE_SPRITE:
    Vector vecOffset;
    VectorMultiply(g_SpriteCenterOffset[prop.m_DetailModel], prop.m_flScale,
                   vecOffset);
...
//-----------------------------------------------------------------------------
static void ComputeMaxDirectLighting(DetailObjectLump_t &prop, Vector *maxcolor,
                                     int iThread) {
  ...
  // NOTE: See version 10 for a method where we choose a normal based on
  // whichever one produces the maximum possible illumination. This appeared to
  // work better on e3_town, so I'm trying it now; hopefully it'll be good for
  // all cases.
  int j;
  for (j = 0; j < lights.Count(); ++j) {
    ...
    VectorMA(maxcolor[dl->light.style],
             out.m_flFalloff.m128_f32[0] * out.m_flDot[0].m128_f32[0],
...
static void ComputeLightmapColorFromAverage(dface_t *pFace,
  ...
  for (int maps = 0; maps < MAXLIGHTMAPS && pFace->styles[maps] != 255;
       ++maps) {
...
static void ComputeLightmapColorPointSample(dface_t *pFace,
                                            directlight_t *pSkylight,
                                            Vector2D const &luv, float scale,
                                            Vector pColor[MAX_LIGHTSTYLES]) {
  ...
  ColorRGBExp32 *pLightmap =
      (ColorRGBExp32 *)&pdlightdata->Base()[pFace->lightofs];
...
public:
  ...
  virtual bool EnumerateLeaf(int leaf, Ray_t const &ray, float start, float end,
                             int context) {
    bool hit = false;
    ...
    for (int i = 0; i < pLeaf->numleaffaces; ++i) {
      ...
      // Don't test a surface that is farther away from the closest found
      // intersection
    ...
    Vector2D luxelCoord;
    dface_t *pDispFace;
    StaticDispMgr()->ClipRayToDispInLeaf(s_DispTested[m_iThread], ray, leaf,
                                         dist, pDispFace, luxelCoord);
    if (dist < m_HitFrac) {
      m_HitFrac = dist;
...
private:
  bool TestPointAgainstSurface(Vector const &pt, dface_t *pFace,
    ...
    if (s < pFace->m_LightmapTextureMinsInLuxels[0] ||
    ...
    if (ds > pFace->m_LightmapTextureSizeInLuxels[0] ||
        dt > pFace->m_LightmapTextureSizeInLuxels[1])
      return false;
...
bool CastRayInLeaf(int iThread, const Vector &start, const Vector &end,
                   int leafIndex, float *pFraction, Vector *pNormal) {
  pFraction[0] = 1.0f;
  ...
  float dist;
  Vector normal;
  StaticDispMgr()->ClipRayToDispInLeaf(s_DispTested[iThread], ray, leafIndex,
                                       dist, &normal);
  if (dist < pFraction[0]) {
    pFraction[0] = dist;
    *pNormal = normal;
...
//-----------------------------------------------------------------------------
void CalcRayAmbientLighting(int iThread, const Vector &vStart,
                            const Vector &vEnd, float tanTheta,
  ...
  // compute the approximate radius of a circle centered around the intersection
  // point
  float dist = ray.m_Delta.Length() * tanTheta * surfEnum.m_HitFrac;

  // until 20" we use the point sample, then blend in the average until we're
  // covering 40" This is attempting to model the ray as a cone - in the ideal
  // case we'd simply sample all luxels in the intersection of the cone with the
  // surface.  Since we don't have surface neighbor information computed we'll
  // just approximate that sampling with a blend between a point sample and the
  // face average. This yields results that are similar in that aliasing is
  // reduced at distance while point samples provide accuracy for intersections
  // with near geometry
  ...
  if (scaleAvg != 0) {
    ComputeLightmapColorFromAverage(surfEnum.m_pSurface, pSkyLight, scaleAvg,
                                    color);
  }
  if (scaleSample != 0) {
    ComputeLightmapColorPointSample(surfEnum.m_pSurface, pSkyLight,
                                    surfEnum.m_LuxelCoord, scaleSample, color);
  }
}
...
//-----------------------------------------------------------------------------
static void ComputeAmbientLightingAtPoint(int iThread, const Vector &origin,
...
void ComputeIndirectLightingAtPoint(Vector &position, Vector &normal,
                                    ...
                                    bool force_fast, bool bIgnoreNormals) {
  Ray_t ray;
  CLightSurface surfEnum(iThread);
  ...
  for (int j = 0; j < nSamples; j++) {
    ...
    if (surfEnum.m_pSurface->styles[0] == 255 ||
        surfEnum.m_pSurface->lightofs < 0) {
    ...
    if (!surfEnum.m_bHasLuxel) {
      ColorRGBExp32 *pAvgLightmapColor =
          dface_AvgLightColor(surfEnum.m_pSurface, 0);
    ...
    float invLengthSqr =
        1.0f /
        (1.0f + ((vEnd - position) * surfEnum.m_HitFrac / 128.0).LengthSqr());
    // Include falloff using invsqrlaw.
    VectorMultiply(lightmapColor,
                   invLengthSqr * dtexdata[pTex->texdata].reflectivity,
                   lightmapColor);
...
static void ComputeAmbientLighting(int iThread, DetailObjectLump_t &prop,
                                   Vector color[MAX_LIGHTSTYLES]) {
  Vector origin, normal;
  ComputeWorldCenter(prop, origin, normal);
...
static void ComputeLighting(DetailObjectLump_t &prop, int iThread) {
  ...
  for (int i = 1; i < MAX_LIGHTSTYLES; ++i) {
    ...
    if ((totalColor[0] != 0.0f) || (totalColor[1] != 0.0f) ||
      ...
      VectorToColorRGBExp32(totalColor,
                            (*s_pDetailPropLightStyleLump)[j].m_Lighting);
...
static int UnserializeDetailProps(DetailObjectLump_t *&pProps) {
  GameLumpHandle_t handle =
      g_GameLumps.GetGameLumpHandle(GAMELUMP_DETAIL_PROPS);

  if (g_GameLumps.GetGameLumpVersion(handle) != GAMELUMP_DETAIL_PROPS_VERSION)
    return 0;

  // Unserialize
  CUtlBuffer buf(g_GameLumps.GetGameLump(handle),
                 g_GameLumps.GameLumpSize(handle), CUtlBuffer::READ_ONLY);
...
//-----------------------------------------------------------------------------
static void
WriteDetailLightingLump(int lumpID, int lumpVersion,
                        CUtlVector<DetailPropLightstylesLump_t> &lumpData) {
...
static void WriteDetailLightingLumps(void) {
  WriteDetailLightingLump(GAMELUMP_DETAIL_PROP_LIGHTING,
                          ...
                          s_DetailPropLightStyleLumpLDR);
  WriteDetailLightingLump(GAMELUMP_DETAIL_PROP_LIGHTING_HDR,
                          GAMELUMP_DETAIL_PROP_LIGHTING_HDR_VERSION,
                          s_DetailPropLightStyleLumpHDR);
}

// need to do this so that if we are building HDR data, the LDR data is intact,
// and vice versa.s
void UnserializeDetailPropLighting(
    int lumpID, int lumpVersion,
    CUtlVector<DetailPropLightstylesLump_t> &lumpData) {
  GameLumpHandle_t handle = g_GameLumps.GetGameLumpHandle(lumpID);
...
void VMPI_ProcessDetailPropWU(int iThread, int iWorkUnit, MessageBuffer *pBuf) {
  CUtlVector<DetailPropLightstylesLump_t> *pDetailPropLump =
      s_pDetailPropLightStyleLump;
  ...
  for (int i = 0; i < prop.m_LightStyleCount; i++) {
    DetailPropLightstylesLump_t *l =
        &pDetailPropLump->Element(i + prop.m_LightStyles);
...
void ComputeDetailPropLighting(int iThread) {
  ...
  if (g_bHDR) {
    UnserializeDetailPropLighting(GAMELUMP_DETAIL_PROP_LIGHTING,
                                  GAMELUMP_DETAIL_PROP_LIGHTING_VERSION,
                                  s_DetailPropLightStyleLumpLDR);
  } else {
    UnserializeDetailPropLighting(GAMELUMP_DETAIL_PROP_LIGHTING_HDR,
                                  GAMELUMP_DETAIL_PROP_LIGHTING_HDR_VERSION,
                                  s_DetailPropLightStyleLumpHDR);
  }
