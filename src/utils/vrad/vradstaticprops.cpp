...
struct colorTexel_t {
  ...
  float m_fDistanceToTri; // If we are outside of the triangle, how far away is
                          // it?
  bool m_bValid;
...
struct Rasterizer {
  ...
  Rasterizer(Vector2D t0, Vector2D t1, Vector2D t2, size_t resX, size_t resY)
      : mT0(t0), mT1(t1), mT2(t2), mResX(resX), mResY(resY),
        mUvStepX(1.0f / resX), mUvStepY(1.0f / resY) {
    Build();
  }
  ...
  inline size_t GetLinearPos(const CUtlVector<Location>::iterator &it) const {
    // Given an iterator, return what the linear position in the buffer would be
    // for the data.
...
private:
  ...
  // It could be large. This is a memory/speed tradeoff. We could instead
  // generate them on demand.
  CUtlVector<Location> mRasterizedLocations;
};

//-----------------------------------------------------------------------------
inline Vector ComputeBarycentric(Vector2D _edgeC, Vector2D _edgeA,
                                 Vector2D _edgeB, float _dAA, float _dAB,
                                 float _dBB, float _invDenom) {
...
void Rasterizer::Build() {
  ...
  // We puff the interesting area up by 1 so we can hit an inflated region for
  // the necessary bilerp data. If we wanted to support better texturing (almost
  ...
  memset(mRasterizedLocations.Base(), 0,
         mRasterizedLocations.Count() * sizeof(Location));

  // Computing Barycentrics adapted from here
  ...
  for (int j = iMinY; j <= iMaxY; ++j) {
    for (int i = iMinX; i <= iMaxX; ++i) {
      ...
      Vector barycentric = ComputeBarycentric(testPt - mT0, edgeA, edgeB, dAA,
                                              dAB, dBB, invDenom);

      // Test whether the point is inside the triangle.
      // MCJOHNTODO: Edge rules and whatnot--right now we re-rasterize points on
      // the edge.
      Location &newLoc = mRasterizedLocations[linearPos++];
      newLoc.barycentric = barycentric;
      newLoc.uv = testPt;

      newLoc.insideTriangle = (barycentric.x >= 0.0f && barycentric.x <= 1.0f &&
...
static void ConvertTexelDataToTexture(
    unsigned int _resX, unsigned int _resY, ImageFormat _destFmt,
    const CUtlVector<colorTexel_t> &_srcTexels, CUtlMemory<byte> *_outTexture);

// Such a monstrosity. :(
static void GenerateLightmapSamplesForMesh(
    const matrix3x4_t &_matPos, const matrix3x4_t &_matNormal, int _iThread,
    int _skipProp, int _nFlags, int _lightmapResX, int _lightmapResY,
    studiohdr_t *_pStudioHdr, mstudiomodel_t *_pStudioModel,
...
static void DumpLightmapLinear(const char *_dstFilename,
                               const CUtlVector<colorTexel_t> &_srcTexels,
...
private:
  // VMPI stuff.
#ifdef MPI
  static void VMPI_ProcessStaticProp_Static(int iThread, uint64 iStaticProp,
                                            MessageBuffer *pBuf);
  static void VMPI_ReceiveStaticPropResults_Static(uint64 iStaticProp,
                                                   MessageBuffer *pBuf,
                                                   int iWorker);
  void VMPI_ProcessStaticProp(int iThread, int iStaticProp,
                              MessageBuffer *pBuf);
  void VMPI_ReceiveStaticPropResults(int iStaticProp, MessageBuffer *pBuf,
                                     int iWorker);
#endif
...
private:
  ...
  struct StaticPropDict_t {
    ...
    CUtlBuffer m_VtxBuf;
    CUtlVector<int> m_textureShadowIndex; // each texture has an index if this
  ...
  struct CStaticProp {
    ...
    // Note that all lightmaps for a given prop share the same resolution (and
    // format)--and there can be multiple lightmaps per prop (if there are
    // multiple pieces--the watercooler is an example). This is effectively
    // because there's not a good way in hammer for a prop to say "this should
    // be the resolution of each of my sub-pieces."
  ...
  void
  ApplyLightingToStaticProp(int iStaticProp, CStaticProp &prop,
...
IVradStaticPropMgr *StaticPropMgr() { return &g_StaticPropMgr; }
...
bool LoadVTXFile(char const *pModelName, const studiohdr_t *pStudioHdr,
  ...
  OptimizedModel::FileHeader_t *pVtxHdr =
      (OptimizedModel::FileHeader_t *)buf.Base();
...
PositionFromIndex(const mstudio_meshvertexdata_t *vertData,
                  mstudiomesh_t *pMesh,
                  OptimizedModel::StripGroupHeader_t *pStripGroup, int i) {
  OptimizedModel::Vertex_t *pVert = pStripGroup->pVertex(i);
  return vertData->Position(pVert->origMeshVertID);
}

//-----------------------------------------------------------------------------
// Purpose: Writes a glview text file containing the collision surface in
// question Input  : *pCollide -
//			*pFilename -
//-----------------------------------------------------------------------------
void DumpCollideToGlView(vcollide_t *pCollide, const char *pFilename) {
  ...
  for (int i = 0; i < pCollide->solidCount; ++i) {
    Vector *outVerts;
    int vertCount =
        s_pPhysCollision->CreateDebugMesh(pCollide->solids[i], &outVerts);
    ...
    for (int i = 0; i < triCount; i++) {
      ...
      fprintf(fp, "%6.3f %6.3f %6.3f %.2f %.3f %.3f\n", outVerts[vert].x,
              outVerts[vert].y, outVerts[vert].z, fr, fg, fb);
      vert++;
      fprintf(fp, "%6.3f %6.3f %6.3f %.2f %.3f %.3f\n", outVerts[vert].x,
              outVerts[vert].y, outVerts[vert].z, fr, fg, fb);
      vert++;
...
public:
  ...
  unsigned char *LoadVTFRGB8888(const char *pName, int *pWidth, int *pHeight,
    ...
    *pClampV = (pTex->Flags() & TEXTUREFLAGS_CLAMPT) ? true : false;
    unsigned char *pDstImage = new unsigned char[ImageLoader::GetMemRequired(
        iWidth, iHeight, 1, dstFormat, false)];

    if (!ImageLoader::ConvertImageFormat(pSrcImage, srcFormat, pDstImage,
                                         dstFormat, iWidth, iHeight, 0, 0)) {
      delete[] pDstImage;
      return NULL;
    }
          ...
          if (pBaseTexture) {
            ...
            if (pBaseTextureName) {
              ...
              unsigned char *pImageBits =
                  LoadVTFRGB8888(pBaseTextureName, &w, &h, &bClampU, &bClampV);
              if (pImageBits) {
  ...
  int AddMaterialEntry(int shadowTextureIndex, const Vector2D &t0,
  ...
  // HACKHACK: Compute the average coverage for this triangle by sampling the
  ...
  int SampleMaterial(int materialIndex, const Vector &coords, bool bBackface) {
    ...
    const alphatexture_t &tex =
        m_Textures.Element(m_MaterialEntries[materialIndex].textureIndex);
    if (bBackface && !tex.allowBackface)
      return 0;
    Vector2D uv =
        coords.x * mat.uv[0] + coords.y * mat.uv[1] + coords.z * mat.uv[2];
...
void CVradStaticPropMgr::CreateCollisionModel(char const *pModelName) {
  ...
  if (LoadStudioCollisionModel(pModelName, bufphy)) {
    ...
    s_pPhysCollision->VCollideLoad(pCollide, header.solidCount,
                                   (const char *)bufphy.PeekGet(),
                                   bufphy.TellPut() - bufphy.TellGet());
  ...
  memcpy(m_StaticPropDict[i].m_pStudioHdr, (studiohdr_t *)buf.Base(),
         buf.Size());

  if (!LoadVTXFile(pModelName, m_StaticPropDict[i].m_pStudioHdr,
                   m_StaticPropDict[i].m_VtxBuf)) {
  ...
  if (g_bTextureShadows) {
    ...
    if (bHasFlag || bForced) {
      ...
      m_StaticPropDict[i].m_textureShadowIndex.AddMultipleToTail(
          pHdr->numtextures);
      g_ShadowTextureList.LoadAllTexturesForModel(
          pHdr, m_StaticPropDict[i].m_textureShadowIndex.Base());
    }
  }
}
...
void CVradStaticPropMgr::UnserializeModels(CUtlBuffer &buf) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    m_StaticProps[i].m_bLightingOriginValid =
        (lump.m_Flags & STATIC_PROP_USE_LIGHTING_ORIGIN) > 0;
    ...
    // Changed this from using DXT1 to RGB888 because the compression artifacts
    // were pretty nasty.
...
void CVradStaticPropMgr::UnserializeStaticProps() {
  ...
  if (g_GameLumps.GetGameLumpVersion(handle) != GAMELUMP_STATIC_PROPS_VERSION) {
    Warning("Cannot load the static props... encountered a stale map version. "
            "Skipping static props.\n");
    return;
  }
...
void CVradStaticPropMgr::Init() {
  ...
  if (!s_pPhysCollision) {
    Error("Unable to get '%s' for physics interface.",
...
//-----------------------------------------------------------------------------
// Trace from a vertex to each direct light source, accumulating its
// contribution.
//-----------------------------------------------------------------------------
void ComputeDirectLightingAtPoint(Vector &position, Vector &normal,
  ...
  for (directlight_t *dl = activelights; dl != NULL; dl = dl->next) {
    ...
    GatherSampleLightSSE(sampleOutput, dl, -1, adjusted_pos4, &normal4, 1,
                         iThread, nLFlags | GATHERLFLAGS_FORCE_FAST,
                         static_prop_id_to_skip, flEpsilon);

    VectorMA(outColor,
             sampleOutput.m_flFalloff.m128_f32[0] *
                 sampleOutput.m_flDot[0].m128_f32[0],
             dl->light.intensity, outColor);
  }
}

//-----------------------------------------------------------------------------
// Takes the results from a ComputeLighting call and applies it to the static
// prop in question.
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::ApplyLightingToStaticProp(
    int iStaticProp, CStaticProp &prop,
    const CComputeStaticPropLightingResults *pResults) {
  if (pResults->m_ColorVertsArrays.Count() == 0 &&
      pResults->m_ColorTexelsArrays.Count() == 0)
    return;
  ...
  for (int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID) {
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
      ...
      const CUtlVector<colorVertex_t> *colorVerts =
          pResults->m_ColorVertsArrays.Count()
              ? pResults->m_ColorVertsArrays[iCurColorVertsArray++]
              : nullptr;
      const CUtlVector<colorTexel_t> *colorTexels =
          pResults->m_ColorTexelsArrays.Count()
              ? pResults->m_ColorTexelsArrays[iCurColorTexelsArray++]
              : nullptr;
      ...
      for (int nLod = 0; nLod < pVtxHdr->numLODs; nLod++) {
        ...
        for (int nMesh = 0; nMesh < pStudioModel->nummeshes; ++nMesh) {
          ...
          for (int nGroup = 0; nGroup < pVtxMesh->numStripGroups; ++nGroup) {
            ...
            if (colorVerts) {
              prop.m_MeshData[nMeshIdx].m_VertexColors.AddMultipleToTail(
                  pStripGroup->numVerts);
              prop.m_MeshData[nMeshIdx].m_nLod = nLod;

              for (int nVertex = 0; nVertex < pStripGroup->numVerts;
                   ++nVertex) {
                int nIndex = pMesh->vertexoffset +
                             pStripGroup->pVertex(nVertex)->origMeshVertID;

                Assert(nIndex < pStudioModel->numvertices);
                prop.m_MeshData[nMeshIdx].m_VertexColors[nVertex] =
                    (*colorVerts)[nIndex].m_Color;
              }
            }

            if (colorTexels) {
              // TODO: Consider doing this work in the worker threads, because
              // then we distribute it.
              ConvertTexelDataToTexture(
                  prop.m_LightmapImageWidth, prop.m_LightmapImageHeight,
                  prop.m_LightmapImageFormat, (*colorTexels),
                  &prop.m_MeshData[nMeshIdx].m_TexelsEncoded);

              if (g_bDumpPropLightmaps) {
                char buffer[_MAX_PATH];
                V_snprintf(buffer, _MAX_PATH - 1,
                           "staticprop_lightmap_%d_%.0f_%.0f_%.0f_%s_%d_%d_%d_%"
                           "d_%d.tga",
                           iStaticProp, prop.m_Origin.x, prop.m_Origin.y,
                           prop.m_Origin.z, dict.m_pStudioHdr->pszName(),
                           bodyID, modelID, nLod, nMesh, nGroup);

                for (int i = 0; buffer[i]; ++i) {
                  if (buffer[i] == '/' || buffer[i] == '\\')
                    buffer[i] = '-';
                }
                DumpLightmapLinear(buffer, (*colorTexels),
                                   prop.m_LightmapImageWidth,
...
// sources at each ray termination. Use the winding data to distribute the
// unique vertexes into the rendering layout.
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::ComputeLighting(
    CStaticProp &prop, int iThread, int prop_index,
    CComputeStaticPropLightingResults *pResults) {
  ...
  const bool withVertexLighting =
      (prop.m_Flags & STATIC_PROP_NO_PER_VERTEX_LIGHTING) == 0;
  const bool withTexelLighting =
      (prop.m_Flags & STATIC_PROP_NO_PER_TEXEL_LIGHTING) == 0;
  ...
  const int skip_prop = (g_bDisablePropSelfShadowing ||
                         (prop.m_Flags & STATIC_PROP_NO_SELF_SHADOWING))
                            ? prop_index
                            : -1;
  const int nFlags = (prop.m_Flags & STATIC_PROP_IGNORE_NORMALS)
                         ? GATHERLFLAGS_IGNORE_NORMALS
                         : 0;
  ...
  for (int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID) {
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
      ...
      if (withTexelLighting) {
        CUtlVector<colorTexel_t> *pColorTexelArray =
            new CUtlVector<colorTexel_t>;
        pResults->m_ColorTexelsArrays.AddToTail(pColorTexelArray);
      }

      // light all unique vertexes
      CUtlVector<colorVertex_t> *pColorVertsArray =
          new CUtlVector<colorVertex_t>;
      ...
      for (int meshID = 0; meshID < pStudioModel->nummeshes; ++meshID) {
        ...
        const mstudio_meshvertexdata_t *vertData =
            pStudioMesh->GetVertexData((void *)pStudioHdr);

        Assert(vertData); // This can only return NULL on X360 for now

        // TODO: Move this into its own function. In fact, refactor this whole
        // function.
        if (withTexelLighting) {
          GenerateLightmapSamplesForMesh(
              matPos, matNormal, iThread, skip_prop, nFlags,
              prop.m_LightmapImageWidth, prop.m_LightmapImageHeight, pStudioHdr,
              pStudioModel, pVtxModel, meshID, pResults);
        }

        // If we do lightmapping, we also do vertex lighting as a potential
        // fallback. This may change.
        for (int vertexID = 0; vertexID < pStudioMesh->numvertices;
             ++vertexID) {
          Vector sampleNormal;
          Vector samplePosition;
          // transform position and normal into world coordinate system
          VectorTransform(*vertData->Position(vertexID), matPos,
                          samplePosition);
          ...
          if (PositionInSolid(samplePosition)) {
            ...
            ComputeDirectLightingAtPoint(direct_pos, sampleNormal, directColor,
                                         iThread, skip_prop, nFlags);
            ...
            if (g_bShowStaticPropNormals) {
              ...
              if (numbounce >= 1)
                ComputeIndirectLightingAtPoint(
                    samplePosition, sampleNormal, indirectColor, iThread, true,
            ...
            VectorAdd(directColor, indirectColor,
                      colorVerts[numVertexes].m_Color);
          }
      ...
      if (badVerts.Count() &&
          (prop.m_bLightingOriginValid || badVerts.Count() != numVertexes)) {
        for (int nBadVertex = 0; nBadVertex < badVerts.Count(); nBadVertex++) {
          ...
          if (prop.m_bLightingOriginValid) {
            ...
            for (int nColorVertex = 0; nColorVertex < numVertexes;
                 nColorVertex++) {
              ...
              Vector delta;
              VectorSubtract(colorVerts[nColorVertex].m_Position,
                             badVerts[nBadVertex].m_Position, delta);
          ...
          // sudivide to determine a closer valid point to the bad vertex, and
          // re-light
          Vector midPosition;
          int numIterations = 20;
          while (--numIterations > 0) {
            VectorAdd(bestPosition, badVerts[nBadVertex].m_Position,
                      midPosition);
          ...
          Vector directColor;
          ComputeDirectLightingAtPoint(bestPosition,
                                       badVerts[nBadVertex].m_Normal,
                                       directColor, iThread);

          Vector indirectColor;
          ComputeIndirectLightingAtPoint(bestPosition,
                                         badVerts[nBadVertex].m_Normal,
                                         indirectColor, iThread, true);

          // save results, not changing valid status
          // to ensure this offset position is not considered as a viable
          // candidate
          colorVerts[badVerts[nBadVertex].m_ColorVertex].m_Position =
              bestPosition;
          VectorAdd(directColor, indirectColor,
                    colorVerts[badVerts[nBadVertex].m_ColorVertex].m_Color);
        }
      }
...
void CVradStaticPropMgr::SerializeLighting() {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    size = sizeof(HardwareVerts::FileHeader_t) +
           m_StaticProps[i].m_MeshData.Count() *
    ...
    HardwareVerts::FileHeader_t *pVhvHdr =
        (HardwareVerts::FileHeader_t *)utlBuf.Base();

    // align to start of vertex data
    unsigned char *pVertexData =
        (unsigned char *)(sizeof(HardwareVerts::FileHeader_t) +
                          m_StaticProps[i].m_MeshData.Count() *
                              sizeof(HardwareVerts::MeshHeader_t));
    ...
    pVhvHdr->m_nChecksum =
        m_StaticPropDict[m_StaticProps[i].m_ModelIdx].m_pStudioHdr->checksum;
    ...
    for (int n = 0; n < pVhvHdr->m_nMeshes; n++) {
      ...
      pMesh->m_nVertexes =
          m_StaticProps[i].m_MeshData[n].m_VertexColors.Count();
    ...
    pVertexData =
        (unsigned char *)((unsigned int)pVertexData - (unsigned int)pVhvHdr);
    pVertexData = (unsigned char *)pVhvHdr +
                  ALIGN_TO_POW2((unsigned int)pVertexData, 512);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    for (int j = 0; j < m_StaticProps[i].m_MeshData.Count(); j++) {
      totalTexelSizeBytes +=
          m_StaticProps[i].m_MeshData[j].m_TexelsEncoded.Count();
    }

    // allocate a buffer with enough padding for alignment
    size = sizeof(HardwareTexels::FileHeader_t) +
           m_StaticProps[i].m_MeshData.Count() *
               sizeof(HardwareTexels::MeshHeader_t) +
           totalTexelSizeBytes + 2 * kAlignment;

    utlBuf.EnsureCapacity(size);
    Q_memset(utlBuf.Base(), 0, size);

    HardwareTexels::FileHeader_t *pVhtHdr =
        (HardwareTexels::FileHeader_t *)utlBuf.Base();

    // align start of texel data
    unsigned char *pTexelData =
        (unsigned char *)(sizeof(HardwareTexels::FileHeader_t) +
                          m_StaticProps[i].m_MeshData.Count() *
                              sizeof(HardwareTexels::MeshHeader_t));
    pTexelData = (unsigned char *)pVhtHdr +
                 ALIGN_TO_POW2((unsigned int)pTexelData, kAlignment);
    ...
    for (int n = 0; n < pVhtHdr->m_nMeshes; n++) {
      ...
      Q_memcpy(pTexelData,
               m_StaticProps[i].m_MeshData[n].m_TexelsEncoded.Base(),
               m_StaticProps[i].m_MeshData[n].m_TexelsEncoded.Count());
      pTexelData += m_StaticProps[i].m_MeshData[n].m_TexelsEncoded.Count();
    }

    pTexelData =
        (unsigned char *)((unsigned int)pTexelData - (unsigned int)pVhtHdr);
    pTexelData = (unsigned char *)pVhtHdr +
                 ALIGN_TO_POW2((unsigned int)pTexelData, kAlignment);

    AddBufferToPak(GetPakFile(), filename, (void *)pVhtHdr,
                   pTexelData - (unsigned char *)pVhtHdr, false);
  }
}

#ifdef MPI
void CVradStaticPropMgr::VMPI_ProcessStaticProp_Static(int iThread,
                                                       uint64 iStaticProp,
                                                       MessageBuffer *pBuf) {
  g_StaticPropMgr.VMPI_ProcessStaticProp(iThread, iStaticProp, pBuf);
}

void CVradStaticPropMgr::VMPI_ReceiveStaticPropResults_Static(
    uint64 iStaticProp, MessageBuffer *pBuf, int iWorker) {
...
// it to the master.
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::VMPI_ProcessStaticProp(int iThread, int iStaticProp,
                                                MessageBuffer *pBuf) {
...
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::VMPI_ReceiveStaticPropResults(int iStaticProp,
                                                       MessageBuffer *pBuf,
                                                       int iWorker) {
  // Read in the results.
  CComputeStaticPropLightingResults results;
...
void CVradStaticPropMgr::ThreadComputeStaticPropLighting(int iThread,
                                                         void *pUserData) {
  while (1) {
    int j = GetThreadWork();
    if (j == -1)
...
// Must be after all other surface lighting has been computed for the indirect
// sampling.
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::ComputeLighting(int iThread) {
  ...
  // ensure any traces against us are ignored because we have no inherit
  ...
  if (g_bUseMPI) {
    ...
    DistributeWork(count, &CVradStaticPropMgr::VMPI_ProcessStaticProp_Static,
...
void CVradStaticPropMgr::AddPolysForRayTrace(void) {
  ...
  for (int nProp = 0; nProp < count; ++nProp) {
      ...
      if (dict.m_pModel) {
        ...
        for (int nConvex = 0; nConvex < queryModel->ConvexCount(); ++nConvex) {
          for (int nTri = 0; nTri < queryModel->TriangleCount(nConvex);
            ...
            g_RtEnv.AddTriangle(TRACE_ID_STATICPROP | nProp, verts[0], verts[1],
                                verts[2], fullCoverage);
          }
        }
        ...
        g_RtEnv.AddAxisAlignedRectangularSolid(TRACE_ID_STATICPROP | nProp,
                                               prop.m_mins, prop.m_maxs,
                                               fullCoverage);
      }
    ...
    // meshes are deeply hierarchial, divided between three stores, follow the
    // white rabbit body parts -> models -> lod meshes -> strip groups -> strips
    // the vertices and indices are pooled, the trick is knowing the offset to
    // determine your indexed base
    for (int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID) {
      ...
      for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
        ...
        for (int nMesh = 0; nMesh < pStudioModel->nummeshes; ++nMesh) {
          // check if this mesh's material is in the no shadow material name
          // list
          mstudiomesh_t *pMesh = pStudioModel->pMesh(nMesh);
          ...
          for (int check = 0; check < g_NonShadowCastingMaterialStrings.Count();
               check++) {
              ...
              if (pStrip->flags & OptimizedModel::STRIP_IS_TRILIST) {
                for (int i = 0; i < pStrip->numIndices; i += 3) {
                  ...
                  if (shadowTextureIndex >= 0) {
                    if (bInitTriangles) {
                      // add texture space and texture index to material
                      // database now
                      float coverage =
                          g_ShadowTextureList.ComputeCoverageForTriangle(
                      ...
                      materialIndex =
                          dict.m_triangleMaterialIndex[triangleIndex];
                      triangleIndex++;
                    }
                    if (materialIndex >= 0) {
...
struct tl_tri_t {
  ...
  bool operator==(const tl_tri_t &t) const {
    return (p0 == t.p0 && p1 == t.p1 && p2 == t.p2 && n0 == t.n0 &&
            n1 == t.n1 && n2 == t.n2);
  }
};
...
void AddTriVertsToList(CUtlVector<tl_vert_t> &triListVerts, int vertIndex,
                       Vector vertPosition, Vector p0, Vector p1, Vector p2,
...
void CVradStaticPropMgr::BuildTriList(CStaticProp &prop) {
  ...
  for (int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID) {
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
      ...
      // in order for this to be monolithic for the entire prop the list must be
      // segmented
      triListVerts.Purge();
          ...
          for (nStrip = 0; nStrip < pStripGroup->numStrips; nStrip++) {
            ...
            if (pStrip->flags & OptimizedModel::STRIP_IS_TRILIST) {
              for (int i = 0; i < pStrip->numIndices; i += 3) {
                ...
                AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex1,
                                  position1, position1, position2, position3,
                                  normal1, normal2, normal3);
                AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex2,
                                  position2, position1, position2, position3,
                                  normal1, normal2, normal3);
                AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex3,
                                  position3, position1, position2, position3,
                                  normal1, normal2, normal3);
              }
            } else {
              // all tris expected to be discrete tri lists
...
const vertexFileHeader_t *mstudiomodel_t::CacheVertexData(void *pModelData) {
  ...
  if (pVvdHdr->checksum != pActiveStudioHdr->checksum) {
    Error("Error Vertex File %s checksum %d should be %d\n", fileName,
          pVvdHdr->checksum, pActiveStudioHdr->checksum);
  }
...
// ------------------------------------------------------------------------------------------------
inline float ComputeBarycentricDistanceToTri(Vector _barycentricCoord,
                                             Vector2D _v[3]) {
  Vector2D realPos = _barycentricCoord.x * _v[0] + _barycentricCoord.y * _v[1] +
...
static void GenerateLightmapSamplesForMesh(
    const matrix3x4_t &_matPos, const matrix3x4_t &_matNormal, int _iThread,
    int _skipProp, int _flags, int _lightmapResX, int _lightmapResY,
    studiohdr_t *_pStudioHdr, mstudiomodel_t *_pStudioModel,
    OptimizedModel::ModelHeader_t *_pVtxModel, int _meshID,
    CComputeStaticPropLightingResults *_outResults) {
  // Could iterate and gen this if needed.
  int nLod = 0;

  OptimizedModel::ModelLODHeader_t *pVtxLOD = _pVtxModel->pLOD(nLod);

  CUtlVector<colorTexel_t> &colorTexels =
      (*_outResults->m_ColorTexelsArrays.Tail());
  ...
  for (int nGroup = 0; nGroup < pVtxMesh->numStripGroups; ++nGroup) {
    ...
    for (nStrip = 0; nStrip < pStripGroup->numStrips; nStrip++) {
      ...
      // If this hits, re-factor the code to iterate over triangles, and build
      // the triangles from the underlying structures.
      ...
      if (pStrip->flags & OptimizedModel::STRIP_IS_TRILIST) {
        for (int i = 0; i < pStrip->numIndices; i += 3) {
          ...
          Vector modelPos[3] = {*vertData->Position(vertex1),
                                *vertData->Position(vertex2),
                                *vertData->Position(vertex3)};

          Vector modelNormal[3] = {*vertData->Normal(vertex1),
          ...
          for (auto it = rasterizer.begin(); it != rasterizer.end(); ++it) {
            ...
            float ourDistancetoTri =
                ComputeBarycentricDistanceToTri(it->barycentric, texcoord);
  ...
  // Process neighbors to the valid region. Walk through the existing array,
  // look for samples that are not valid but are adjacent to valid samples.
  // Works if we are only bilinearly sampling on the other side. First attempt:
  // Just pretend the triangle was larger and cast a ray from this new world pos
  // as above.
  int linearPos = 0;
  for (int j = 0; j < _lightmapResY; ++j) {
    for (int i = 0; i < _lightmapResX; ++i) {
      ...
      if (colorTexels[linearPos].m_bPossiblyInteresting) {
        ...
        shouldProcess =
            shouldProcess ||
            colorTexels[ComputeLinearPos(i - 1, j - 1, _lightmapResX,
                                         _lightmapResY)]
                .m_bValid // TL
            || colorTexels[ComputeLinearPos(i, j - 1, _lightmapResX,
                                            _lightmapResY)]
                   .m_bValid // T
            || colorTexels[ComputeLinearPos(i + 1, j - 1, _lightmapResX,
                                            _lightmapResY)]
            ...
            || colorTexels[ComputeLinearPos(i - 1, j, _lightmapResX,
                                            _lightmapResY)]
                   .m_bValid // L
            || colorTexels[ComputeLinearPos(i + 1, j, _lightmapResX,
                                            _lightmapResY)]
      ...
      if (shouldProcess) {
        Vector directColor(0, 0, 0), indirectColor(0, 0, 0);

        ComputeDirectLightingAtPoint(colorTexels[linearPos].m_WorldPosition,
                                     colorTexels[linearPos].m_WorldNormal,
                                     directColor, _iThread, _skipProp, _flags);

        if (numbounce >= 1) {
          ComputeIndirectLightingAtPoint(
              colorTexels[linearPos].m_WorldPosition,
              colorTexels[linearPos].m_WorldNormal, indirectColor, _iThread,
              true, (_flags & GATHERLFLAGS_IGNORE_NORMALS) != 0);
        }
...
// ------------------------------------------------------------------------------------------------
static int GetTexelCount(unsigned int _resX, unsigned int _resY,
                         bool _mipmaps) {
  // Because they are unsigned, this is a != check--but if we were to change to
  // ints, this would be the right assert (and it's no worse than != now).
  ...
  // Add in the 1x1 mipmap level, which wasn't hit above. This could be done in
  // the initializer of retVal, but it's more obvious here.
  retVal += 1;
...
// ------------------------------------------------------------------------------------------------
static void FilterFineMipmap(unsigned int _resX, unsigned int _resY,
                             const CUtlVector<colorTexel_t> &_srcTexels,
                             CUtlVector<Vector> *_outLinear) {
  Assert(_outLinear);
    ...
    for (int i = 0; i < _resX; ++i) {
      ...
      for (int offsetJ = -cRadius; offsetJ <= cRadius; ++offsetJ) {
        for (int offsetI = -cRadius; offsetI <= cRadius; ++offsetI) {
          int finalIndex =
              ComputeLinearPos(i + offsetI, j + offsetJ, _resX, _resY);
...
// ------------------------------------------------------------------------------------------------
static void BuildFineMipmap(unsigned int _resX, unsigned int _resY,
                            bool _applyFilter,
                            const CUtlVector<colorTexel_t> &_srcTexels,
                            CUtlVector<RGB888_t> *_outTexelsRGB888,
                            CUtlVector<Vector> *_outLinear) {
  // At least one of these needs to be non-null, otherwise what are we doing
  // here?
  ...
  if (_applyFilter) {
    ...
    for (int i = 0; i < _srcTexels.Count(); ++i) {
      ...
      RGBA8888_t encodedColor;
      VectorToColorRGBExp32(_srcTexels[i].m_Color, rgbColor);
      ConvertRGBExp32ToRGBA8888(&rgbColor, (unsigned char *)&encodedColor,
                                (_outLinear ? (&(*_outLinear)[i]) : NULL));
      // We drop alpha on the floor here, if this were to fire we'd need to
      // consider using a different compressed format.
...
// ------------------------------------------------------------------------------------------------
static void FilterCoarserMipmaps(unsigned int _resX, unsigned int _resY,
                                 CUtlVector<Vector> *_scratchLinear,
                                 CUtlVector<RGB888_t> *_outTexelsRGB888) {
  ...
  // TODO: Should do better filtering for downsampling. But this will work for
  // now.
  while (srcResX > 1 || srcResY > 1) {
    ...
    for (int j = 0; j < srcResY; j += 2) {
      for (int i = 0; i < srcResX; i += 2) {
        ...
        ConvertLinearToRGB888(
            &sample,
            &(*_outTexelsRGB888)[dstOffset + dstCol + dstRow * dstResX]);
        // Also overwrite the srcBuffer to filter the next loop. This is safe
        // because we won't be reading this source value again during this
        // mipmap level.
...
// ------------------------------------------------------------------------------------------------
static void ConvertToDestinationFormat(
    unsigned int _resX, unsigned int _resY, ImageFormat _destFmt,
    const CUtlVector<RGB888_t> &_scratchRBG888, CUtlMemory<byte> *_outTexture) {
  const ImageFormat cSrcImageFormat = IMAGE_FORMAT_RGB888;

  // Converts from the scratch RGB888 buffer, which should be fully filled out
  // to the output texture.
  int destMemoryUsage =
      ImageLoader::GetMemRequired(_resX, _resY, 1, _destFmt, true);
  ...
  if (cSrcImageFormat != _destFmt) {
    while (srcResX > 1 || srcResY > 1) {
      ...
      ImageLoader::ConvertImageFormat(
          (unsigned char *)(&_scratchRBG888[srcOffset]), cSrcImageFormat,
          (*_outTexture).Base() + dstOffset, _destFmt, srcResX, srcResY);

      // Then update offsets for the next mipmap level.
      srcOffset += GetTexelCount(srcResX, srcResY, false);
      dstOffset +=
          ImageLoader::GetMemRequired(srcResX, srcResY, 1, _destFmt, false);
    ...
    ImageLoader::ConvertImageFormat(
        (unsigned char *)_scratchRBG888.Base() + srcOffset, cSrcImageFormat,
        (*_outTexture).Base() + dstOffset, _destFmt, srcResX, srcResY);
  } else {
...
static void ConvertTexelDataToTexture(
  ...
  BuildFineMipmap(_resX, _resY, true, _srcTexels, &scratchRGB888,
                  &scratchLinear);
  FilterCoarserMipmaps(_resX, _resY, &scratchLinear, &scratchRGB888);
  ConvertToDestinationFormat(_resX, _resY, _destFmt, scratchRGB888,
                             _outTexture);
}
...
static void DumpLightmapLinear(const char *_dstFilename,
  ...
  TGAWriter::WriteTGAFile(_dstFilename, _width, _height, IMAGE_FORMAT_BGR888,
