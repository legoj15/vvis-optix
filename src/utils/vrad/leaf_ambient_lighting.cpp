...
// TODO: it's CRAZY how much lighting code we share with the engine. It should
// all be shared code.
float Engine_WorldLightDistanceFalloff(const dworldlight_t *wl,
                                       const Vector &delta) {
  Assert(wl->type == emit_surface);
...
void AddEmitSurfaceLights(const Vector &vStart, Vector lightBoxColor[6]) {
  ...
  for (int iLight = 0; iLight < *pNumworldlights; iLight++) {
    ...
    float flAngleScale =
        Engine_WorldLightAngle(wl, wl->normal, vDeltaNorm, vDeltaNorm);
...
void ComputeAmbientFromSphericalSamples(int iThread, const Vector &vStart,
                                        Vector lightBoxColor[6]) {
  ...
  // Now add direct light from the emit_surface lights. These go in the ambient
  // cube because there are a ton of them and they are often so dim that they
  // get filtered out by r_worldlightmin.
...
bool IsLeafAmbientSurfaceLight(dworldlight_t *wl) {
  ...
  static const float g_flWorldLightMinEmitSurfaceDistanceRatio =
      (InvRSquared(Vector(0, 0, 512)));
...
public:
  ...
  // do a couple of tracing heuristics to eliminate points that are inside
  // detail brushes or underneath displacement surfaces in the leaf return once
  // we have a valid point, use the center if one can't be computed quickly
  void GenerateLeafSamplePosition(int leafIndex,
                                  const CUtlVector<dplane_t> &leafPlanes,
    ...
    for (int i = 0; i < 1000 && !bValid; i++) {
      ...
      for (int j = leafPlanes.Count(); --j >= 0 && bValid;) {
        float d = DotProduct(leafPlanes[j].normal, samplePosition) -
                  leafPlanes[j].dist;
        if (d < DIST_EPSILON) {
    ...
    if (!bValid) {
      // didn't generate a valid sample point, just use the center of the leaf
      // bbox
      samplePosition =
          (Vector(pLeaf->mins[0], pLeaf->mins[1], pLeaf->mins[2]) +
           Vector(pLeaf->maxs[0], pLeaf->maxs[1], pLeaf->maxs[2])) *
          0.5f;
    }
  }
...
// add the sample to the list.  If we exceed the maximum number of samples, the
// worst sample will be discarded.  This has the effect of converging on the
// best samples when enough are added.
void AddSampleToList(CUtlVector<ambientsample_t> &list,
                     const Vector &samplePosition, Vector *pCube) {
...
int CubeDeltaGammaSpace(Vector *pCube0, Vector *pCube1) {
  int maxDelta = 0;
  // do this comparison in gamma space to try and get a perceptual basis for the
  // compare
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 3; j++) {
...
// worldspace optionally skip one of the entries in the list
void Mod_LeafAmbientColorAtPos(Vector *pOut, const Vector &pos,
                               const CUtlVector<ambientsample_t> &list,
                               int skipIndex) {
  for (int i = 0; i < 6; i++) {
    pOut[i].Init();
  }
  ...
  for (int i = 0; i < list.Count(); i++) {
    if (i == skipIndex)
      continue;
    // do an inverse squared distance weighted average of the samples to
...
// basically this is an intersection routine that returns a distance between the
// boxes
float AABBDistance(const Vector &mins0, const Vector &maxs0,
...
void ComputeAmbientForLeaf(int iThread, int leafID,
  ...
  list.RemoveAll();
  // this heuristic tries to generate at least one sample per volume (chosen to
  // be similar to the size of a player) in the space
...
//-----------------------------------------------------------------------------
void VMPI_ReceiveLeafAmbientResults(uint64 leafID, MessageBuffer *pBuf,
                                    int iWorker) {
  // Decode the results.
  int nSamples;
  ...
  if (nSamples) {
    pBuf->read(g_LeafAmbientSamples[leafID].Base(),
               nSamples * sizeof(ambientsample_t));
  }
}
#endif
...
void ComputePerLeafAmbientLighting() {
  ...
  Msg("%d of %d (%d%% of) surface lights went in leaf ambient cubes.\n",
      nInAmbientCube, nSurfaceLights,
      nSurfaceLights ? ((nInAmbientCube * 100) / nSurfaceLights) : 0);
  ...
  if (g_bUseMPI) {
    ...
    DistributeWork(numleafs, VMPI_ProcessLeafAmbient,
                   VMPI_ReceiveLeafAmbientResults);
  } else
#endif
  {
...
#undef NO_THREAD_NAMES
  ...
  for (int leafID = 0; leafID < numleafs; leafID++) {
    ...
    if (!list.Count()) {
      ...
      g_pLeafAmbientIndex->Element(leafID).firstAmbientSample =
          g_pLeafAmbientLighting->Count();
      // compute the samples in disk format.  Encode the positions in 8-bits
      // using leaf bounds fractions
      for (int i = 0; i < list.Count(); i++) {
        int outIndex = g_pLeafAmbientLighting->AddToTail();
        dleafambientlighting_t &light =
            g_pLeafAmbientLighting->Element(outIndex);

        light.x = Fixed8Fraction(list[i].pos.x, dleafs[leafID].mins[0],
                                 dleafs[leafID].maxs[0]);
        light.y = Fixed8Fraction(list[i].pos.y, dleafs[leafID].mins[1],
                                 dleafs[leafID].maxs[1]);
        light.z = Fixed8Fraction(list[i].pos.z, dleafs[leafID].mins[2],
                                 dleafs[leafID].maxs[2]);
        light.pad = 0;
  ...
  for (int i = 0; i < numleafs; i++) {
    // UNDONE: Do this dynamically in the engine instead.  This will allow us to
    // sample across leaf boundaries always which should improve the quality of
    // lighting in general
