...
//==========================================================================//
// Direct Lighting diagnostic counters (per-thread, summed at report time).
// Zero synchronization overhead — each thread writes only its own slot.
//==========================================================================//
long g_nLightsSkippedDistance[MAX_TOOL_THREADS];
...
public:
  ...
  // Adds the normal if unique. Otherwise, returns the normal's index into
  // m_Normals.
...
CNormalList::CNormalList() : m_Normals(128) {
  for (int i = 0; i < sizeof(m_NormalGrid) / sizeof(m_NormalGrid[0][0][0]);
       i++) {
  ...
  for (i = 0, f = g_pFaces; i < numfaces; i++, f++) {
    ...
    for (j = 0; j < f->numedges; j++) {
      ...
      for (k = 0; k < vertexref[n]; k++) {
        ...
        // if this face doens't have a displacement -- don't consider
        ...
        // add normal if >= threshold or its a displacement surface (this is
        // only if the original face is a displacement)
        if (fn->bHasDisp) {
          ...
          if ((f->smoothingGroups == 0) &&
              (g_pFaces[vertexface[n][k]].smoothingGroups == 0)) {
            ...
            unsigned int smoothingGroup =
                (f->smoothingGroups &
                 g_pFaces[vertexface[n][k]].smoothingGroups);
...
void SaveVertexNormals(void) {
  ...
  for (i = 0; i < numfaces; i++) {
    ...
    for (j = 0; j < f->numedges; j++) {
      ...
      g_vertnormalindices[g_numvertnormalindices] =
          (unsigned short)normalList.FindOrAddNormal(vNormal);
  ...
  memcpy(g_vertnormals, normalList.m_Normals.Base(),
         sizeof(g_vertnormals[0]) * normalList.m_Normals.Size());
}
...
void ErrorLightInfo(const char *s, lightinfo_t *l) {
  ...
  if (w != NULL) {
    ...
    Warning(
        "%s at (%g, %g, %g)\n\tmaterial=%s\n", s, (double)vecCenter.x,
        (double)vecCenter.y, (double)vecCenter.z,
...
void CalcFaceVectors(lightinfo_t *l) {
  ...
  // Solve[ { x * w00 + y * w01 + z * w02 - s == 0, x * w10 + y * w11 + z * w12
  // - t == 0, A * x + B * y + C * z + D == 0 }, { x, y, z } ] Rule(x,(  C*s*w11
  // - B*s*w12 + B*t*w02 - C*t*w01 + D*w02*w11 - D*w01*w12) / (+ A*w01*w12 -
  // A*w02*w11 + B*w02*w10 - B*w00*w12 + C*w00*w11 - C*w01*w10 )), Rule(y,(
  // A*s*w12 - C*s*w10 + C*t*w00 - A*t*w02 + D*w00*w12 - D*w02*w10) / (+
  // A*w01*w12 - A*w02*w11 + B*w02*w10 - B*w00*w12 + C*w00*w11 - C*w01*w10 )),
  // Rule(z,(  B*s*w10 - A*s*w11 + A*t*w01 - B*t*w00 + D*w01*w10 - D*w00*w11) /
  // (+ A*w01*w12 - A*w02*w11 + B*w02*w10 - B*w00*w12 + C*w00*w11 - C*w01*w10
  // ))))

  Vector luxelSpaceCross;

  luxelSpaceCross[0] = tex->lightmapVecsLuxelsPerWorldUnits[1][1] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][2] -
                       tex->lightmapVecsLuxelsPerWorldUnits[1][2] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][1];
  luxelSpaceCross[1] = tex->lightmapVecsLuxelsPerWorldUnits[1][2] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][0] -
                       tex->lightmapVecsLuxelsPerWorldUnits[1][0] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][2];
  luxelSpaceCross[2] = tex->lightmapVecsLuxelsPerWorldUnits[1][0] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][1] -
                       tex->lightmapVecsLuxelsPerWorldUnits[1][1] *
                           tex->lightmapVecsLuxelsPerWorldUnits[0][0];
  ...
  if (fabs(det) < 1.0e-20) {
    ...
    // invert the matrix
    l->luxelToWorldSpace[0][0] =
        (l->facenormal[2] * l->worldToLuxelSpace[1][1] -
         l->facenormal[1] * l->worldToLuxelSpace[1][2]) /
        det;
    l->luxelToWorldSpace[1][0] =
        (l->facenormal[1] * l->worldToLuxelSpace[0][2] -
         l->facenormal[2] * l->worldToLuxelSpace[0][1]) /
        det;
    l->luxelOrigin[0] = -(l->facedist * luxelSpaceCross[0]) / det;
    l->luxelToWorldSpace[0][1] =
        (l->facenormal[0] * l->worldToLuxelSpace[1][2] -
         l->facenormal[2] * l->worldToLuxelSpace[1][0]) /
        det;
    l->luxelToWorldSpace[1][1] =
        (l->facenormal[2] * l->worldToLuxelSpace[0][0] -
         l->facenormal[0] * l->worldToLuxelSpace[0][2]) /
        det;
    l->luxelOrigin[1] = -(l->facedist * luxelSpaceCross[1]) / det;
    l->luxelToWorldSpace[0][2] =
        (l->facenormal[1] * l->worldToLuxelSpace[1][0] -
         l->facenormal[0] * l->worldToLuxelSpace[1][1]) /
        det;
    l->luxelToWorldSpace[1][2] =
        (l->facenormal[0] * l->worldToLuxelSpace[0][1] -
         l->facenormal[1] * l->worldToLuxelSpace[0][0]) /
        det;
    l->luxelOrigin[2] = -(l->facedist * luxelSpaceCross[2]) / det;

    // adjust for luxel offset
    VectorMA(l->luxelOrigin, -tex->lightmapVecsLuxelsPerWorldUnits[0][3],
             l->luxelToWorldSpace[0], l->luxelOrigin);
    VectorMA(l->luxelOrigin, -tex->lightmapVecsLuxelsPerWorldUnits[1][3],
             l->luxelToWorldSpace[1], l->luxelOrigin);
  }
...
void WriteCoordWinding(FILE *out, lightinfo_t *l, winding_t *w, Vector &color) {
  ...
  for (i = 0; i < w->numpoints; i++) {
    ...
    fprintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", pos[0], pos[1],
            pos[2], color[0] / 256, color[1] / 256, color[2] / 256);
  }
}
...
void DumpFaces(lightinfo_t *pLightInfo, int ndxFace) {
  ...
  for (int ndxEdge = 0; ndxEdge < pLightInfo->face->numedges; ndxEdge++) {
    ...
    Vector p1, p2;
    VectorAdd(dvertexes[EdgeVertex(pLightInfo->face, ndxEdge)].point,
              pLightInfo->modelorg, p1);
    VectorAdd(dvertexes[EdgeVertex(pLightInfo->face, ndxEdge + 1)].point,
              pLightInfo->modelorg, p2);
    ...
    CmdLib_FPrintf(out, "%f %f %f %f %f %f\n", p1[0], p1[1], p1[2],
                   n1[0] * 0.5 + 0.5, n1[1] * 0.5 + 0.5, n1[2] * 0.5 + 0.5);

    CmdLib_FPrintf(out, "%f %f %f %f %f %f\n", p2[0], p2[1], p2[2],
                   n2[0] * 0.5 + 0.5, n2[1] * 0.5 + 0.5, n2[2] * 0.5 + 0.5);

    CmdLib_FPrintf(
        out, "%f %f %f %f %f %f\n", centroid[0] + pLightInfo->modelorg[0],
        centroid[1] + pLightInfo->modelorg[1],
        centroid[2] + pLightInfo->modelorg[2], fn->facenormal[0] * 0.5 + 0.5,
        fn->facenormal[1] * 0.5 + 0.5, fn->facenormal[2] * 0.5 + 0.5);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool BuildFacesamplesAndLuxels_DoFast(lightinfo_t *pLightInfo,
  ...
  pFaceLight->worldAreaPerLuxel =
      1.0 / (sqrt(DotProduct(pTex->lightmapVecsLuxelsPerWorldUnits[0],
  ...
  pFaceLight->sample =
      (sample_t *)calloc(pFaceLight->numsamples, sizeof(*pFaceLight->sample));
  ...
  for (int t = 0; t < height; t++) {
    for (int s = 0; s < width; s++) {
      ...
      LuxelSpaceToWorld(pLightInfo, pSamples->coord[0], pSamples->coord[1],
                        pSamples->pos);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool BuildSamplesAndLuxels_DoFast(lightinfo_t *pLightInfo,
                                  facelight_t *pFaceLight, int ndxFace) {
  ...
  // build samples for a "displacement"
    ...
    return StaticDispMgr()->BuildDispSamplesAndLuxels_DoFast(
        pLightInfo, pFaceLight, ndxFace);
  }
}
...
bool BuildFacesamples(lightinfo_t *pLightInfo, facelight_t *pFaceLight) {
  ...
  for (int t = 0; t < height && pLightmapWinding; t++) {
    ...
    // hack - need a separate epsilon for lightmap space since ON_EPSILON is for
    // texture space
    ClipWindingEpsilon(pLightmapWinding, tNorm, dist, ON_EPSILON / 16.0f,
                       &pWindingT1, &pWindingT2);

    for (int s = 0; s < width && pWindingT2; s++) {
      dist = s + sampleOffset;

      // lop off a sample in the s dimension, and put it in ws2
      // hack - need a separate epsilon for lightmap space since ON_EPSILON is
      // for texture space
      ClipWindingEpsilon(pWindingT2, sNorm, dist, ON_EPSILON / 16.0f,
                         &pWindingS1, &pWindingS2);
      ...
      if (pWindingS2) {
        ...
        Vector center;
        pSamples->area = WindingAreaAndBalancePoint(pWindingS2, center) *
                         pFaceLight->worldAreaPerLuxel;
        ...
        if (g_bDumpPatches ||
            (do_extra &&
             pSamples->area < pFaceLight->worldAreaPerLuxel - EQUAL_EPSILON)) {
          //
          // convert the winding from lightmaps space to world for debug
          // rendering and sub-sampling
          //
          Vector worldPos;
          for (int ndxPt = 0; ndxPt < pWindingS2->numpoints; ndxPt++) {
            LuxelSpaceToWorld(pLightInfo, pWindingS2->p[ndxPt].x,
                              pWindingS2->p[ndxPt].y, worldPos);
      ...
      //
      // if winding T2 still exists free it and set it equal S1 (the rest of the
      // row minus the sample just created)
      //
      if (pWindingT2) {
    ...
    //
    // if the original lightmap winding exists free it and set it equal to T1
    // (the rest of the winding not cut into samples)
    //
    if (pLightmapWinding) {
  ...
  memcpy(pFaceLight->sample, samples,
         pFaceLight->numsamples * sizeof(*pFaceLight->sample));
...
//-----------------------------------------------------------------------------
// Purpose: Free any windings used by this facelight. It's currently assumed
// they're not needed again
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool BuildSamples(lightinfo_t *pLightInfo, facelight_t *pFaceLight,
                  int ndxFace) {
  // build samples for a "face"
  if (pLightInfo->face->dispinfo == -1) {
...
//          primitive type
//-----------------------------------------------------------------------------
bool BuildLuxels(lightinfo_t *pLightInfo, facelight_t *pFaceLight,
                 int ndxFace) {
...
directlight_t **g_ClusterLightFlat = nullptr;
int *g_ClusterLightOffsets = nullptr;
...
static void ParseLightGeneric(entity_t *e, directlight_t *dl) {
  ...
  if (target[0]) { // point towards target
    ...
    SetupLightNormalFromProps(QAngle(angles.x, angles.y, angles.z), angle,
                              pitch, dl->light.normal);
  }
  if (g_bHDR)
...
static void SetLightFalloffParams(entity_t *e, directlight_t *dl) {
  ...
  if (d50) {
    ...
    if (d0 < d50) {
      Warning("light has _fifty_percent_distance of %f but "
              "_zero_percent_distance of %f\n",
              d50, d0);
      d0 = 2.0 * d50;
    }
    float a = 0, b = 1, c = 0;
    ...
    // it it possible that the parameters couldn't be used because of enforing
    // monoticity. If so, rescale so at least the 50 percent value is right
    ...
    if (IntForKey(e, "_hardfalloff")) {
      ...
      dl->m_flStartFadeDistance =
          0.75 * d0 +
          0.25 *
              d50; // start fading 3/4 way between 50 and 0. could allow adjust
    } else {
      // now, we will find the point at which the 1/x term reaches its maximum
      // value, and prevent the light from going past there. If a user specifes
      // an extreme falloff, the quadratic will start making the light brighter
      // at some distance. We handle this by fading it from the minimum
      // brightess point down to zero at 10x the minimum distance
      if (fabs(a) > 0.) {
    ...
    if (dl->light.constant_attn < EQUAL_EPSILON &&
        dl->light.linear_attn < EQUAL_EPSILON &&
        dl->light.quadratic_attn < EQUAL_EPSILON)
      dl->light.constant_attn = 1;

    // scale intensity for unit 100 distance
    float ratio = (dl->light.constant_attn + 100 * dl->light.linear_attn +
                   100 * 100 * dl->light.quadratic_attn);
    if (ratio > 0) {
...
static void ParseLightSpot(entity_t *e, directlight_t *dl) {
  ...
  if ((dl->light.stopdot == 180) && (dl->light.stopdot2 == 180)) {
    ...
    if (dl->light.stopdot > 90) {
      Warning("WARNING: light_spot at (%i %i %i) has inner angle larger than "
              "90 degrees! Clamping to 90...\n",
              (int)dl->light.origin[0], (int)dl->light.origin[1],
              (int)dl->light.origin[2]);
      dl->light.stopdot = 90;
    }
    ...
    if (dl->light.stopdot2 > 90) {
      Warning("WARNING: light_spot at (%i %i %i) has outer angle larger than "
              "90 degrees! Clamping to 90...\n",
              (int)dl->light.origin[0], (int)dl->light.origin[1],
              (int)dl->light.origin[2]);
...
// NOTE: This is just a heuristic.  It traces a finite number of rays to find
// sky NOTE: Full vis is necessary to make this 100% correct.
bool CanLeafTraceToSky(int iLeaf) {
  // UNDONE: Really want a point inside the leaf here.  Center is a guess, may
  // not be in the leaf UNDONE: Clip this to each plane bounding the leaf to
  // guarantee
  ...
  for (int j = 0; j < NUMVERTEXNORMALS; j += 4) {
    ...
    delta.LoadAndSwizzle(g_anorms[j],
                         g_anorms[min(j + 1, NUMVERTEXNORMALS - 1)],
                         g_anorms[min(j + 2, NUMVERTEXNORMALS - 1)],
                         g_anorms[min(j + 3, NUMVERTEXNORMALS - 1)]);
...
void BuildVisForLightEnvironment(void) {
  ...
  for (int iLeaf = 0; iLeaf < numleafs; ++iLeaf) {
    ...
    for (int iLeafFace = 0; iLeafFace < dleafs[iLeaf].numleaffaces;
         ++iLeafFace) {
  ...
  // Second pass to set flags on leaves that don't contain sky, but touch leaves
  // that contain sky.
  byte pvs[MAX_MAP_CLUSTERS / 8];
  ...
  // Must set the bits in a separate pass so as to not flood-fill LEAF_FLAGS_SKY
  // everywhere pLeafbits is a bit array of all leaves that need to be marked as
  // seeing sky
  for (int iLeaf = 0; iLeaf < numleafs; ++iLeaf) {
    // If this leaf has light (3d skybox) in it, then don't bother
    if (dleafs[iLeaf].flags & LEAF_FLAGS_SKY)
    ...
    // If this is a 3D skybox leaf, then we don't care if it was previously a 2D
    // skybox leaf
...
static char *ValueForKeyWithDefault(entity_t *ent, char *key,
...
void CreateDirectLights(void) {
  ...
  for (i = 0; i < uiPatchCount; i++) {
    ...
    if (VectorAvg(p->baselight) >= dlight_threshold) {
      ...
      VectorScale(p->baselight,
                  lightscale * p->area * p->scale[0] * p->scale[1] /
                      p->basearea,
                  dl->light.intensity);
  ...
  // Sort activelights by cluster for cache-friendly PVS byte access.
  // PVS is a bitfield indexed by cluster, so lights in nearby clusters
  // access nearby bytes, improving cache locality during PVS checks.
  if (numdlights > 1) {
    // Collect into temp array
    CUtlVector<directlight_t *> sortedLights;
    sortedLights.EnsureCapacity(numdlights);
    for (directlight_t *dl = activelights; dl != NULL; dl = dl->next) {
      sortedLights.AddToTail(dl);
    }

    // Sort by cluster (ascending)
    struct {
      static int Compare(const void *a, const void *b) {
        directlight_t *la = *(directlight_t **)a;
        directlight_t *lb = *(directlight_t **)b;
        return la->light.cluster - lb->light.cluster;
      }
    } sorter;
    qsort(sortedLights.Base(), sortedLights.Count(), sizeof(directlight_t *),
          sorter.Compare);

    // Re-link
    activelights = sortedLights[0];
    for (int j = 0; j < sortedLights.Count() - 1; j++) {
      sortedLights[j]->next = sortedLights[j + 1];
    }
...
#define NSAMPLES_SUN_AREA_LIGHT                                                \
  30 // number of samples to take for an
     // non-point sun light

// Helper function - gathers light from sun (emit_skylight)
void GatherSampleSkyLightSSE(SSE_sampleLightOutput_t &out, directlight_t *dl,
                             int facenum, FourVectors const &pos,
                             FourVectors *pNormals, int normalCount,
                             int iThread, int nLFlags,
                             int static_prop_index_to_ignore, float flEpsilon) {
  ...
  for (int d = 0; d < nsamples; d++) {
    ...
    TestLine_DoesHitSky(pos, delta4, &fractionVisible, true,
...
void GatherSampleAmbientSkySSE(SSE_sampleLightOutput_t &out, directlight_t *dl,
                               int facenum, FourVectors const &pos,
                               FourVectors *pNormals, int normalCount,
                               int iThread, int nLFlags,
  ...
  for (int j = 0; j < nsky_samples; j++) {
    ...
    possibleHitCount[0] =
        AddSIMD(AndSIMD(validity, Four_Ones), possibleHitCount[0]);
    ...
    for (int i = 1; i < normalCount; i++) {
      ...
      possibleHitCount[i] =
          AddSIMD(AndSIMD(AndSIMD(validity, validity2), Four_Ones),
                  possibleHitCount[i]);
    }
    ...
    TestLine_DoesHitSky(surfacePos, delta, &fractionVisible, true,
                        static_prop_index_to_ignore);
    for (int i = 0; i < normalCount; i++) {
  ...
  out.m_flFalloff = Four_Ones;
  for (int i = 0; i < normalCount; i++) {
    // now scale out the missing parts of the hemisphere of this bump basis
    // vector
...
// Helper function - gathers light from area lights, spot lights, and point
// lights
void GatherSampleStandardLightSSE(SSE_sampleLightOutput_t &out,
                                  directlight_t *dl, int facenum,
                                  FourVectors const &pos, FourVectors *pNormals,
                                  int normalCount, int iThread, int nLFlags,
                                  int static_prop_index_to_ignore,
                                  float flEpsilon) {
  bool bIgnoreNormals = (nLFlags & GATHERLFLAGS_IGNORE_NORMALS) != 0;
  ...
  if (bHasHardFalloff) {
    fltx4 notPastFadeDist =
        CmpLeSIMD(dist, ReplicateX4(dl->m_flEndFadeDistance));
  ...
  case emit_spotlight:
    ...
    // pow is fixed point, so this isn't the most accurate, but it doesn't need
    // to be
  ...
  bool bSkipVisibility = (nLFlags & GATHERLFLAGS_SKIP_VISIBILITY) != 0;
  if (bSkipVisibility) {
    // Store ray source for deferred GPU batching
    out.m_raySource = src;
    out.m_flDot[0] = dot;
  } else {
    fltx4 fractionVisible = Four_Ones;
    // For point/spot lights, src hasn't been modified since delta/dist were
    // computed, so we can reuse the pre-computed normalized direction and
    // length.  Surface lights offset src by DIST_EPSILON, so fall back to
    // the original TestLine which recomputes direction from start/stop.
    if (dl->light.type != emit_surface) {
      TestLine_PrecomputedDir(pos, delta, dist, &fractionVisible,
                              static_prop_index_to_ignore);
    } else {
...
void GatherSampleLightSSE(SSE_sampleLightOutput_t &out, directlight_t *dl,
  ...
  case emit_skylight:
    GatherSampleSkyLightSSE(out, dl, facenum, pos, pNormals, normalCount,
                            iThread, nLFlags, static_prop_index_to_ignore,
                            flEpsilon);
    break;
  ...
  // position) we don't want it to contribute to *any* of the bumped lightmaps.
  // It glows in disturbing ways if we don't do this.
...
void AddSampleToPatch(sample_t *s, LightingValue_t &light, int facenum) {
  ...
  //
  // fixed the sample position and normal -- need to find the equiv pos, etc to
  // set up patches
  //
  ...
  for (patch = &g_Patches.Element(g_FacePatches.Element(facenum)); patch;
       patch = pNextPatch) {
    // next patch
    pNextPatch = NULL;
    ...
    VectorMA(patch->samplelight, s->area, light.m_vecLighting,
             patch->samplelight);
  ...
  if (smoothing_threshold != 1) {
    ...
    for (j = 0; j < f->numedges; j++) {
      ...
      */
      ...
      // Test center to sample vector for inclusion between center to vertex
      // vectors (Use dot product of vectors)
      if (a1 >= 0.0 && a2 >= 0.0) {
        // calculate distance from edge to pos
        Vector temp;
        float scale;

        // Interpolate between the center and edge normals based on sample
        // position
        scale = 1.0 - a1 - a2;
...
void GetPhongNormal(int facenum, FourVectors const &spot,
  ...
  if (smoothing_threshold != 1) {
    ...
    for (j = 0; j < f->numedges; ++j) {
      ...
      fltx4 resultMask =
          AndSIMD(CmpGeSIMD(a1, Four_Zeros), CmpGeSIMD(a2, Four_Zeros));

      if (!TestSignSIMD(resultMask))
        continue;

      // Store the old phong normal to avoid overwriting already computed phong
      // normals
      ...
      phongnormal.x = AddSIMD(AndSIMD(resultMask, phongnormal.x),
                              AndNotSIMD(resultMask, oldPhongNormal.x));
      phongnormal.y = AddSIMD(AndSIMD(resultMask, phongnormal.y),
                              AndNotSIMD(resultMask, oldPhongNormal.y));
      phongnormal.z = AddSIMD(AndSIMD(resultMask, phongnormal.z),
                              AndNotSIMD(resultMask, oldPhongNormal.z));
    }
        ...
        for (int iBump = 0; iBump < 4; ++iBump) {
          if (iBump == 0 || (iBump > 0 && bBumpped)) {
            for (int iSample = 0; iSample < pFaceLight->numsamples; ++iSample) {
              ...
              WriteWinding(
                  pFileSamples[iStyle][iBump], pSample->w,
                  pFaceLight->light[iStyle][iBump][iSample].m_vecLighting);
              if (bDumpNormals) {
                WriteNormal(pFileSamples[iStyle][iBump], pSample->pos,
                            pSample->normal, 15.0f, pSample->normal * 255.0f);
              }
            }
          }
        }
...
//-----------------------------------------------------------------------------
static inline void AllocateLightstyleSamples(facelight_t *fl, int styleIndex,
                                             int numnormals) {
  for (int n = 0; n < numnormals; ++n) {
    fl->light[styleIndex][n] =
        (LightingValue_t *)calloc(fl->numsamples, sizeof(LightingValue_t));
  }
}
...
//-----------------------------------------------------------------------------
static void ComputeIlluminationPointAndNormalsSSE(lightinfo_t const &l,
                                                  FourVectors const &pos,
                                                  FourVectors const &norm,
  ...
  if (computeNormals) {
    ...
    for (int b = 0; b < NUM_BUMP_VECTS; ++b) {
      pInfo->m_PointNormals[b + 1].LoadAndSwizzle(bv[0][b], bv[1][b], bv[2][b],
                                                  bv[3][b]);
    }
  }
...
//-----------------------------------------------------------------------------
static void GatherSampleLightAt4Points(SSE_SampleInfo_t &info, int sampleIdx,
                                       int numSamples) {
  SSE_sampleLightOutput_t out;

  // Pre-compute whether all samples share the same cluster.
  // When true (common for flat surfaces), we only need one PVS check
  // per light instead of up to 4.
  bool allSameCluster =
      (numSamples <= 1) ||
  ...
  for (int li_idx = 0;; li_idx++) {
    ...
    if (dl->facenum == -1 &&
      ...
      if (minDist2 > endFade2) {
        g_nLightsSkippedDistance[cpuThread]++;
        continue;
      }
    }
    ...
    if (skipLight) {
      g_nLightsSkippedZeroDot[cpuThread]++;
      continue;
    }
...
//-----------------------------------------------------------------------------
static void
ResampleLightAt4Points(SSE_SampleInfo_t &info, int lightStyleIndex, int flags,
                       LightingValue_t pLightmap[4][NUM_BUMP_VECTS + 1]) {
  SSE_sampleLightOutput_t out;
...
static int SupersampleLightAtPoint(lightinfo_t &l, SSE_SampleInfo_t &info,
                                   int sampleIndex, int lightStyleIndex,
                                   LightingValue_t *pLight, int flags) {
  ...
  if (flags & NON_AMBIENT_ONLY) {
    ...
    for (int s = 0; s < 4; ++s) {
      // make sure the coordinate is inside of the sample's winding and when
      // normalizing below use the number of samples used, not just
      // numsamples and some of them will be skipped if they are not inside
      // of the winding
      ...
      LuxelSpaceToWorld(&l, superSampleLightCoord[0], superSampleLightCoord[1],
                        superSamplePosition);

      // A winding should exist only if the sample wasn't a uniform luxel,
      // or if g_bDumpPatches is true.
      int invalidBits = 0;
      if (sample.w &&
      ...
      ComputeIlluminationPointAndNormalsSSE(l, superSamplePosition,
                                            superSampleNormal, &info, 4);
...
//-----------------------------------------------------------------------------
static void ComputeLightmapGradients(SSE_SampleInfo_t &info,
                                     bool const *pHasProcessedSample,
  ...
  for (int i = 0; i < fl->numsamples; i++) {
    ...
    for (int n = 0; n < info.m_NormalCount; ++n) {
      ...
      if (sample.t > 0) {
        if (sample.s > 0)
          gradient[i] =
              max(gradient[i], fabs(pIntensity[j] - pIntensity[j - 1 - w]));
      ...
      if (sample.s > 0)
        gradient[i] = max(gradient[i], fabs(pIntensity[j] - pIntensity[j - 1]));
      if (sample.s < w - 1)
        gradient[i] = max(gradient[i], fabs(pIntensity[j] - pIntensity[j + 1]));
    }
  }
...
static inline void ComputeLuxelIntensity(SSE_SampleInfo_t &info, int sampleIdx,
  ...
  for (int n = 0; n < info.m_NormalCount; ++n) {
    ...
    pSampleIntensity[n * info.m_LightmapSize + destIdx] =
...
//-----------------------------------------------------------------------------
static void BuildSupersampleFaceLights(lightinfo_t &l, SSE_SampleInfo_t &info,
                                       int lightstyleIndex) {
  ...
  // This is used to compute a simple gradient computation of the light
  // samples We're going to store the maximum intensity of all bumped
  // samples at each sample location
  float *pGradient =
  ...
  // discontinuities (large light intensity gradients) as a clue that we
  // should probably be supersampling in that area. Because the
  // supersampling operation will cause lighting changes, we've found that
  // it's good to re-check the gradients again and see if any other areas
  // should be supersampled as a result of the previous pass. Keep going
  // until all the gradients are reasonable or until we hit a max number of
  // passes
  ...
  while (do_anotherpass && pass <= extrapasses) {
    ...
    for (int i = 0; i < info.m_pFaceLight->numsamples; ++i) {
      ...
      g_nSSGradientTotal[info.m_iThread]++;
      ...
      int directSupersampleCount = SupersampleLightAtPoint(
          l, info, i, lightstyleIndex, pDirectLight, NON_AMBIENT_ONLY);
...
static void InitSampleInfo(lightinfo_t const &l, int iThread,
                           SSE_SampleInfo_t &info);

#ifdef VRAD_RTX_CUDA_SUPPORT
//-----------------------------------------------------------------------------
// CPU Supersampling + Patch Lights for GPU Path
//
...
void FinalizeAndSupersample(int iThread, int facenum) {
  ...
  // Full CPU supersampling — all gradient passes, all lights
...
static void InitSampleInfo(lightinfo_t const &l, int iThread,
  ...
  info.m_NormalCount =
      (info.m_pTexInfo->flags & SURF_BUMPLIGHT) ? NUM_BUMP_VECTS + 1 : 1;
  ...
  info.m_NumSampleGroups = (info.m_NumSamples & 0x3)
                               ? (info.m_NumSamples / 4) + 1
                               : (info.m_NumSamples / 4);
...
void BuildFacelights(int iThread, int facenum) {
  ...
  // perf-critical code.
  if (g_pIncremental) {
    // Both threads will be accessing this so it needs to be protected or
    // else thread A will load it in and thread B will increment it but its
    // increment will be overwritten by thread A when thread A writes it
    // back.
    ThreadLock();
    ++g_iCurFace;
    ThreadUnlock();
  ...
  // check for patches for this face.  If none it must be degenerate.
  // Ignore.
  if (g_FacePatches.Element(facenum) == g_FacePatches.InvalidIndex())
    return;
...
void BuildPatchLights(int facenum) {
  ...
  // push up sampled light to parents (children always exist first in the
  // list)
  CPatch *pNextPatch;
  for (patch = &g_Patches.Element(g_FacePatches.Element(facenum)); patch;
       patch = pNextPatch) {
  ...
  for (patch = &g_Patches.Element(g_FacePatches.Element(facenum)); patch;
    ...
    if (patch->child1 != g_Patches.InvalidIndex()) {
      ...
      VectorMA(patch->totallight.light[0], s2, child2->totallight.light[0],
               patch->totallight.light[0]);
...
void PrecompLightmapOffsets() {
  ...
  // NOTE: We store avg face light data in this lump *before* the lightmap
  // data itself in *reverse order* of the way the lightstyles appear in the
  // styles array.
  for (facenum = 0; facenum < numfaces; facenum++) {
    ...
    int nLuxels = (f->m_LightmapTextureSizeInLuxels[0] + 1) *
                  (f->m_LightmapTextureSizeInLuxels[1] + 1);
    if (needsBumpmap) {
  ...
  // The incremental lighting code needs us to preserve the contents of
  // dlightdata since it only recomposites lighting for faces that have
  // lights that touch them.
...
#define CONDITION(a, b, c)                                                     \
  ...
  for (i = 0; i < 3; i++) {
    ...
    // This channel is too bright. . take half of the amount that we are
    // over and add it to the other two channel.
...
static void LinearToBumpedLightmap(const float *linearColor,
  ...
  if (*(int *)&bumpAverage[0] != 0 && *(int *)&bumpAverage[1] != 0 &&
      *(int *)&bumpAverage[2] != 0) {
    // fast path when we know that we don't have to worry about divide by
    // zero.
  ...
  Vector check =
      (correctedBumpColor1 + correctedBumpColor2 + correctedBumpColor3) / 3.0f;

  ColorClampBumped(correctedBumpColor1, correctedBumpColor2,
                   correctedBumpColor3);
...
// This matches the engine's conversion, so the lighting result is
// consistent.
//-----------------------------------------------------------------------------
void ConvertRGBExp32ToRGBA8888(const ColorRGBExp32 *pSrc, unsigned char *pDst,
                               Vector *_optOutLinear) {
  Vector linearColor;

  // convert from ColorRGBExp32 to linear space
  linearColor[0] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->r,
                                    ((ColorRGBExp32 *)pSrc)->exponent);
  linearColor[1] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->g,
                                    ((ColorRGBExp32 *)pSrc)->exponent);
  linearColor[2] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->b,
                                    ((ColorRGBExp32 *)pSrc)->exponent);
...
void ConvertRGBExp32ToLinear(const ColorRGBExp32 *pSrc, Vector *pDst) {
  ...
  (*pDst)[0] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->r,
                                ((ColorRGBExp32 *)pSrc)->exponent);
  (*pDst)[1] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->g,
                                ((ColorRGBExp32 *)pSrc)->exponent);
  (*pDst)[2] = TexLightToLinear(((ColorRGBExp32 *)pSrc)->b,
                                ((ColorRGBExp32 *)pSrc)->exponent);
}

//-----------------------------------------------------------------------------
// Converts a linear color value (suitable for combining linearly) to an
// RBGA8888 value expected by the engine.
//-----------------------------------------------------------------------------
void ConvertLinearToRGBA8888(const Vector *pSrcLinear, unsigned char *pDst) {
  ...
  // the colorspace version found in the engine, which *is* the same
  // sequence here
