...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 3; ++i) {
    if (!Q_IsAbsolutePath(argv[i + 1])) {
      Q_snprintf(pBuf[i], sizeof(pBuf[i]), "%s\\%s", pCurrentDirectory,
                 argv[i + 1]);
      pFileName[i] = pBuf[i];
    } else {
  ...
  unsigned char *pImage1Tmp = new unsigned char[ImageLoader::GetMemRequired(
      width1, height1, 1, imageFormat1, false)];
  unsigned char *pImage2Tmp = new unsigned char[ImageLoader::GetMemRequired(
      width2, height2, 1, imageFormat2, false)];

  buf1.SeekGet(CUtlBuffer::SEEK_HEAD, 0);
  if (!TGALoader::Load(pImage1Tmp, buf1, width1, height1, imageFormat1, 2.2f,
                       false)) {
    printf("error loading %s\n", pFileName[0]);
    exit(-1);
  }
  ...
  if (!TGALoader::Load(pImage2Tmp, buf2, width2, height2, imageFormat2, 2.2f,
                       false)) {
    printf("error loading %s\n", pFileName[1]);
    exit(-1);
  }

  unsigned char *pImage1 = new unsigned char[ImageLoader::GetMemRequired(
      width1, height1, 1, IMAGE_FORMAT_ABGR8888, false)];
  unsigned char *pImage2 = new unsigned char[ImageLoader::GetMemRequired(
      width2, height2, 1, IMAGE_FORMAT_ABGR8888, false)];
  unsigned char *pDiff = new unsigned char[ImageLoader::GetMemRequired(
      width2, height2, 1, IMAGE_FORMAT_ABGR8888, false)];
  ImageLoader::ConvertImageFormat(pImage1Tmp, imageFormat1, pImage1,
                                  IMAGE_FORMAT_ABGR8888, width1, height1, 0, 0);
  ImageLoader::ConvertImageFormat(pImage2Tmp, imageFormat2, pImage2,
                                  IMAGE_FORMAT_ABGR8888, width2, height2, 0, 0);
  ...
  for (int i = 0; i < width1 * height1; i++) {
    ...
    for (int j = 0; j < sizeInBytes; j++) {
      int idx = i * sizeInBytes + j;
      int d = pImage2[idx] - pImage1[idx];
      pDiff[idx] = d > 0 ? d : -d;
      if (d != 0) {
        pixelDifferent = true;
      }
  ...
  double diffPercent = (double)diffCount / (width1 * height1) * 100.0;

  if (!isDifferent) {
    printf("Files are the same %s %s : not generating %s\n", pFileName[0],
           pFileName[1], pFileName[2]);
    printf("Difference: 0.00%%\n");
    exit(-1);
  } else {
  ...
  if (!TGAWriter::WriteToBuffer(pDiff, outBuffer, width1, height1,
                                dstImageFormat, dstImageFormat)) {
