...
int main(int argc, char *argv[]) {
  ...
  if (fp) {
    if (fgets(dllName, sizeof(dllName), fp)) {
      ...
      if (pModule)
        printf(
            "Loaded alternate VRAD DLL (%s) specified in vrad_rtx.redirect.\n",
            dllName);
      else
        printf("Can't find '%s' specified in vrad_rtx.redirect.\n", dllName);
    }
  ...
  for (int mode = 0; mode < 2; mode++) {
    ...
    if (!pModule) {
      strcpy(dllName, "vrad_rtx_dll.dll");
      pModule = Sys_LoadModule(dllName);
    }
    ...
    if (!fn) {
      printf("vrad_launcher error: can't get factory from vrad_rtx_dll.dll\n");
    ...
    if (!pDLL) {
      printf("vrad_launcher error: can't get IVRadDLL interface from "
             "vrad_rtx_dll.dll\n");
      Sys_UnloadModule(pModule);
      return 3;
    }
