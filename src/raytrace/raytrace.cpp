...
void RayTracingEnvironment::AddTriangle(int32 id, const Vector &v1,
                                        const Vector &v2, const Vector &v3,
                                        const Vector &color, uint16 flags,
                                        int32 materialIndex) {
  ...
  // Store vertices for OptiX GAS building (before ChangeIntoIntersectionFormat)
  TriangleVertices.AddToTail(v1);
  TriangleVertices.AddToTail(v2);
  TriangleVertices.AddToTail(v3);
...
void RayTracingEnvironment::AddAxisAlignedRectangularSolid(
    int id, Vector minc, Vector maxc, const Vector &color) {

  // "far" face
  AddQuad(id, Vector(minc.x, maxc.y, maxc.z), Vector(maxc.x, maxc.y, maxc.z),
          Vector(maxc.x, minc.y, maxc.z), Vector(minc.x, minc.y, maxc.z),
          color);
  // "near" face
  AddQuad(id, Vector(minc.x, maxc.y, minc.z), Vector(maxc.x, maxc.y, minc.z),
          Vector(maxc.x, minc.y, minc.z), Vector(minc.x, minc.y, minc.z),
          color);

  // "left" face
  AddQuad(id, Vector(minc.x, maxc.y, maxc.z), Vector(minc.x, maxc.y, minc.z),
          Vector(minc.x, minc.y, minc.z), Vector(minc.x, minc.y, maxc.z),
          color);
  // "right" face
  AddQuad(id, Vector(maxc.x, maxc.y, maxc.z), Vector(maxc.x, maxc.y, minc.z),
          Vector(maxc.x, minc.y, minc.z), Vector(maxc.x, minc.y, maxc.z),
          color);

  // "top" face
  AddQuad(id, Vector(minc.x, maxc.y, maxc.z), Vector(maxc.x, maxc.y, maxc.z),
          Vector(maxc.x, maxc.y, minc.z), Vector(minc.x, maxc.y, minc.z),
          color);
  // "bot" face
  AddQuad(id, Vector(minc.x, minc.y, maxc.z), Vector(maxc.x, minc.y, maxc.z),
          Vector(maxc.x, minc.y, minc.z), Vector(minc.x, minc.y, minc.z),
          color);
}
...
void RayTracingEnvironment::Trace4Rays(
  ...
  if (msk != -1)
    ...
    // sucky case - can't trace 4 rays at once. in the worst case, need to trace
    // all 4 separately, but usually we will still get 2x, Since our tracer only
    // does 4 at a time, we will have to cover up the undesired rays with the
    // desired ray
    ...
    for (int try_trace = 0; try_trace < 4; try_trace++) {
      if (need_trace[try_trace]) {
        ...
        // now, see if any of the other remaining rays can be handled at the
        // same time.
        ...
        for (int i = 0; i < 4; i++)
          if (need_trace[i] == 2) {
            ...
            SubFloat(rslt_out->HitDistance, i) =
                SubFloat(tmpresults.HitDistance, i);
...
void RayTracingEnvironment::Trace4Rays(
    ...
    RayTracingResult *rslt_out, int32 skip_id,
  ...
  for (int c = 0; c < 3; c++) {
    fltx4 isect_min_t = MulSIMD(
        SubSIMD(ReplicateX4(m_MinBound[c]), rays.origin[c]), OneOverRayDir[c]);
    fltx4 isect_max_t = MulSIMD(
        SubSIMD(ReplicateX4(m_MaxBound[c]), rays.origin[c]), OneOverRayDir[c]);
  ...
  while (1) {
    while (CurNode->NodeType() != KDNODE_STATE_LEAF) // traverse until next leaf
    {
      ...
      CacheOptimizedKDNode const *FrontChild =
          &(OptimizedKDTree[CurNode->LeftChild()]);

      fltx4 dist_to_sep_plane = // dist=(split-org)/dir
          MulSIMD(SubSIMD(ReplicateX4(CurNode->SplittingPlaneValue),
                          rays.origin[split_plane_number]),
                  OneOverRayDir[split_plane_number]);
      active = CmpLeSIMD(TMin, TMax); // mask of which rays are active

      // now, decide how to traverse children. can either do front,back, or do
      // front and push back.
    ...
    if (ntris) {
      ...
      do {
        ...
        TriIntersectData_t const *tri =
            &(OptimizedTriangleList[tnum].m_Data.m_IntersectData);
        if ((mailboxids[mbox_slot] != tnum) &&
            (tri->m_nTriangleID != skip_id)) {
          ...
          // now, check 3 edges
          fltx4 hitc1 =
              AddSIMD(rays.origin[tri->m_nCoordSelect0],
                      MulSIMD(isect_t, rays.direction[tri->m_nCoordSelect0]));
          fltx4 hitc2 =
              AddSIMD(rays.origin[tri->m_nCoordSelect1],
                      MulSIMD(isect_t, rays.direction[tri->m_nCoordSelect1]));

          // do barycentric coordinate check
          fltx4 B0 =
              MulSIMD(ReplicateX4(tri->m_ProjectedEdgeEquations[0]), hitc1);

          B0 =
              AddSIMD(B0, MulSIMD(ReplicateX4(tri->m_ProjectedEdgeEquations[1]),
                                  hitc2));
          B0 = AddSIMD(B0, ReplicateX4(tri->m_ProjectedEdgeEquations[2]));

          did_hit = AndSIMD(did_hit, CmpGeSIMD(B0, FourZeros));

          fltx4 B1 =
              MulSIMD(ReplicateX4(tri->m_ProjectedEdgeEquations[3]), hitc1);
          B1 =
              AddSIMD(B1, MulSIMD(ReplicateX4(tri->m_ProjectedEdgeEquations[4]),
                                  hitc2));

          B1 = AddSIMD(B1, ReplicateX4(tri->m_ProjectedEdgeEquations[5]));
          ...
          if (tri->m_nFlags & FCACHETRI_TRANSPARENT) {
            if (pCallback) {
              ...
              // the projected edge equations are set up such that the vert
              // opposite the first equation is v2, and the vert opposite the
              // second equation is v0 Therefore we pass them back in 1, 2, 0
              // order Also B2 is currently B1 + B0 and needs to be 1 - (B1+B0)
              // in order to be a real barycentric coordinate.  Compute that now
              // and pass it to the callback
              fltx4 b2 = SubSIMD(Four_Ones, B2);
              if (pCallback->VisitTriangle_ShouldContinue(
                      *tri, rays, &did_hit, &B1, &b2, &B0, tnum)) {
                did_hit = Four_Zeros;
              }
            }
          }
          ...
          rslt_out->surface_normal.x =
              OrSIMD(AndSIMD(N.x, did_hit),
                     AndNotSIMD(did_hit, rslt_out->surface_normal.x));
          rslt_out->surface_normal.y =
              OrSIMD(AndSIMD(N.y, did_hit),
                     AndNotSIMD(did_hit, rslt_out->surface_normal.y));
          rslt_out->surface_normal.z =
              OrSIMD(AndSIMD(N.z, did_hit),
...
void RayTracingEnvironment::CalculateTriangleListBounds(int32 const *tris,
                                                        int ntris,
                                                        Vector &minout,
                                                        Vector &maxout) {
...
float RayTracingEnvironment::CalculateCostsOfSplit(
    int split_plane, int32 const *tri_list, int ntris, Vector MinBound,
  ...
  // intersection fromat yet, we can use the CoordSelect0 field of each as a
  // temp.
  nleft = 0;
  nright = 0;
  nboth = 0;
...
void RayTracingEnvironment::RefineNode(int node_number, int32 const *tri_list,
                                       int ntris, Vector MinBound,
                                       Vector MaxBound, int depth) {
  if (ntris < 3) // never split empty lists
  {
    // no point in continuing
    OptimizedKDTree[node_number].Children =
        KDNODE_STATE_LEAF + (TriangleIndexList.Count() << 2);
    ...
    for (int ts = -1; ts < ntris; ts += tri_skip) {
      for (int tv = 0; tv < 3; tv++) {
        ...
        if (ts == -1)
          ...
          if ((trial_splitvalue > MaxBound[axis]) ||
              (trial_splitvalue < MinBound[axis]))
        ...
        if (trial_cost < best_cost) {
          ...
          for (int t = 0; t < ntris; t++) {
            CacheOptimizedTriangle &tri = OptimizedTriangleList[tri_list[t]];
            tri.m_Data.m_GeometryData.m_nTmpData1 =
                tri.m_Data.m_GeometryData.m_nTmpData0;
          }
        }
        if (ts == -1)
          break;
  ...
  if ((cost_of_no_split <= best_cost) || NEVER_SPLIT ||
    ...
    // 		printf("best split was %d at %f (mid=%f,n=%d,
    ...
    // 		printf("node %d split on axis %d at %f, nl=%d nr=%d nb=%d lc=%d
    // rc=%d\n",node_number,
    ...
    if ((ntris < 20) && ((best_nleft == 0) || (best_nright == 0)))
      depth += 100;
    RefineNode(left_child, new_triangle_list, best_nleft + best_nboth, LeftMins,
               LeftMaxes, depth + 1);
    RefineNode(right_child, new_triangle_list + best_nleft,
               best_nright + best_nboth, RightMins, RightMaxes, depth + 1);
...
void RayTracingEnvironment::SetupAccelerationStructure(void) {
  ...
  RefineNode(0, root_triangle_list, OptimizedTriangleList.Count(), m_MinBound,
             m_MaxBound, 0);
...
void RayTracingEnvironment::AddInfinitePointLight(Vector position,
                                                  Vector intensity) {
