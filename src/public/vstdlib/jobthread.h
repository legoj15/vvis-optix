...
enum JobFlags_t {
  ...
  JF_BOOST_THREAD =
      (1 << 1), // Up the thread priority to max allowed while processing task
  JF_SERIAL = (1 << 2), // Job cannot be executed out of order relative to other
                        // "strict" jobs
...
struct ThreadPoolStartParams_t {
  ThreadPoolStartParams_t(bool bIOThreads = false, unsigned nThreads = -1,
      ...
      : bIOThreads(bIOThreads), nThreads(nThreads), fDistribute(fDistribute),
        nStackSize(nStackSize), iThreadPriority(iThreadPriority),
        nThreadsMax(-1) {
    bExecOnThreadPoolThreadsOnly = false;

    bUseAffinityTable = (pAffinities != NULL) && (fDistribute == TRS_TRUE) &&
                        (nThreads != (unsigned)-1);
...
public:
  CJob(JobPriority_t priority = JP_NORMAL)
      : m_status(JOB_STATUS_UNSERVICED), m_ThreadPoolData(JOB_NO_DATA),
        m_priority(priority), m_flags(0), m_pThreadPool(NULL),
        m_CompleteEvent(true), m_iServicingThread(-1) {
    m_szDescription[0] = 0;
  }
  ...
  void IterRangeParallel(                                                      \
    ...
    if (nThreads < 2) {                                                        \
      ...
      while (--nThreads) {                                                     \
        ...
        jobSet += g_pThreadPool->AddCall(pfnProxied, from,                     \
...
#define DEFINE_MEMBER_ITER_RANGE_PARALLEL(N)                                   \
  ...
  void IterRangeParallel(                                                      \
    ...
    if (nThreads < 2) {                                                        \
      ...
      while (--nThreads) {                                                     \
        ITERTYPE2 thisTo = from + nIncrement;                                  \
        jobSet += g_pThreadPool->AddCall(pObject, pfnProxied, from,            \
                                         thisTo FUNC_FUNCTOR_CALL_ARGS_##N);   \
        from = thisTo;                                                         \
      }                                                                        \
      FunctorDirectCall(pObject, pfnProxied, from,                             \
...
template <typename T> class CJobItemProcessor {
public:
    ...
    if (nJobs > 1) {
      ...
      while (i--) {
        jobs[i] = pThreadPool->QueueCall(
            this,
            &CParallelProcessor<ITEM_TYPE, ITEM_PROCESSOR_TYPE>::DoExecute);
      ...
      for (i = 0; i < nJobs; i++) {
        jobs[i]->Abort(); // will either abort ones that never got a thread, or
                          // noop on ones that did
        jobs[i]->Release();
      }
    } else {
      DoExecute();
...
template <class ITEM_PROCESSOR_TYPE> class CParallelLoopProcessor {
public:
...
inline void
ParallelLoopProcess(const char *szDescription, long lBegin, unsigned nItems,
                    void (*pfnProcess)(long const &), void (*pfnBegin)() = NULL,
                    void (*pfnEnd)() = NULL, int nMaxParallel = INT_MAX) {
...
template <class Derived> class CParallelProcessorBase {
protected:
...
inline ThreadHandle_t ThreadExecuteSoloRef(const char *pszName, T1 a1, T2 a2,
                                           T3 a3, T4 a4, T5 a5, T6 a6) {
  return ThreadExecuteSoloImpl(CreateRefCountingFunctor(a1, a2, a3, a4, a5, a6),
                               pszName);
}
