...
public:
  ...
  FileBlockingItem()
      : m_ItemType((FileBlockingWarning_t)0), m_flElapsed(0.0f),
        m_nAccessType(0) {
    SetFileName(NULL);
  }
  ...
  FileBlockingItem(int type, char const *filename, float elapsed,
                   int accessType)
      : m_ItemType((FileBlockingWarning_t)type), m_flElapsed(elapsed),
...
enum FilesystemOpenExFlags_t {
  ...
  FSOPEN_FORCE_TRACK_CRC =
      (1 << 1), // This makes it calculate a CRC for the file (if the file came
...
enum FSAsyncStatus_t {
  ...
  FSASYNC_ERR_RETRY_LATER =
      -7, // Failure for a reason that might be temporary.  You might retry, but
          // not immediately.  (E.g. Network problems)
...
enum FSAsyncFlags_t {
  ...
  FSASYNC_FLAGS_NULLTERMINATE =
      (1 << 3), // allocate an extra byte and null terminate the buffer read in
};
...
struct FileAsyncRequest_t {
  ...
  FSAllocFunc_t pfnAlloc; // custom allocator. can be null. not compatible with
                          // FSASYNC_FLAGS_FREEDATAPTR
};
...
public:
  ...
  const char *RelativePathToFullPath_safe(
    ...
    return RelativePathToFullPath(pFileName, pPathID, pDest, (int)maxLenInChars,
                                  pathFilter, pPathType);
  }

  // Returns the search path, each path is separated by ;s. Returns the length
  // of the string returned Prefer using the GetSearchPath_safe template wrapper
  ...
  /// requests and pull the data from a source of their choosing.  The immediate
  /// use case is to load assets from the CDN via HTTP.
  virtual void AsyncAddFetcher(IAsyncFileFetch * pFetcher) = 0;
  virtual void AsyncRemoveFetcher(IAsyncFileFetch * pFetcher) = 0;

  //------------------------------------
  // Functions to hold a file open if planning on doing multiple reads. Use is
  ...
  // Prefer using the FullPathToRelativePathEx_safe template wrapper to calling
  // this directly
  ...
  template <size_t maxLenInChars>
  bool FullPathToRelativePathEx_safe(const char *pFullpath, const char *pPathId,
                                     OUT_Z_ARRAY char (&pDest)[maxLenInChars]) {
    return FullPathToRelativePathEx(pFullpath, pPathId, pDest,
                                    (int)maxLenInChars);
  }
  ...
  virtual bool RegisterMemoryFile(CMemoryFileBacking * pFile,
                                  CMemoryFileBacking *
                                      *ppExistingFileWithRef) = 0;
  ...
  // this directly
  virtual bool GetCaseCorrectFullPath_Ptr(const char *pFullPath,
                                          OUT_Z_CAP(maxLenInChars) char *pDest,
                                          int maxLenInChars) = 0;
  template <size_t maxLenInChars>
  ...
  // Whether we are allowed to write anywhere (default) or just to our search
  // paths. By default, we can write everywhere--but the game client locks us
  // down to writing in specific places, because remote users can coerce us into
  // writing things down via a clickable URL.
...
public:
  // malloc and free in headers with our janky memdbg system. What could go
  // wrong. Except everything. (this free can't skip memdbg if paired malloc
  // used memdbg)
#include <memdbgon.h>
  CMemoryFileBacking(IFileSystem *pFS)
      : m_pFS(pFS), m_nRegistered(0), m_pFileName(NULL), m_pData(NULL),
        m_nLength(0) {}
  ~CMemoryFileBacking() {
    free((char *)m_pFileName);
    if (m_pData)
      m_pFS->FreeOptimalReadBuffer((char *)m_pData);
  }
#include <memdbgoff.h>
