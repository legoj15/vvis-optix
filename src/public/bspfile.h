...
#define MAX_MAP_VERTS 262144 // raised from 65536 for accurate vertex counting
#define MAX_MAP_VERTNORMALS 256000
#define MAX_MAP_VERTNORMALINDICES 256000
#define MAX_MAP_FACES 262144     // raised from 65536 for countverts mode
#define MAX_MAP_LEAFFACES 262144 // raised from 65536 for countverts mode
#define MAX_MAP_LEAFBRUSHES 65536
#define MAX_MAP_PORTALS 65536
#define MAX_MAP_CLUSTERS 65536
#define MAX_MAP_LEAFWATERDATA 32768
#define MAX_MAP_PORTALVERTS 128000
#define MAX_MAP_EDGES 1024000     // raised from 256000 for countverts mode
#define MAX_MAP_SURFEDGES 2048000 // raised from 512000 for countverts mode
#define MAX_MAP_LIGHTING 0x1000000
#define MAX_MAP_VISIBILITY 0x1000000 // increased BSPVERSION 7
#define MAX_MAP_TEXTURES 1024
#define MAX_MAP_WORLDLIGHTS 32767
...
#define MAX_MAP_PRIMITIVES 131072  // raised from 32768 for countverts mode
#define MAX_MAP_PRIMVERTS 262144   // raised from 65536 for countverts mode
#define MAX_MAP_PRIMINDICES 262144 // raised from 65536 for countverts mode
...
struct lump_t {
  ...
  // this field was char fourCC[4] previously, but was unused, favoring the LUMP
  // IDs above instead. It has been repurposed for compression.  0 implies the
  // lump is not compressed.
...
typedef struct texinfo_s {
  ...
  float lightmapVecsLuxelsPerWorldUnits[2][4]; // [s/t][xyz offset] - length is
...
public:
  ...
  uint32 m_AllowedVerts[ALLOWEDVERTS_SIZE]; // This is built based on the layout
...
// Per-face index into the packed side-ID data lump.
// When -emitsideids is used, each BSP face stores the full list of
// original VMF side IDs that were merged into it.
struct dfacesideid_index_t {
  int firstId; // offset into LUMP_FACE_SIDEIDS_DATA
  int numIds;  // number of side IDs for this face
};
...
struct dareaportal_t {
  DECLARE_BYTESWAP_DATADESC();
  unsigned short m_PortalKey; // Entities have a key called portalnumber (and in
                              // vbsp a variable called areaportalnum) which is
                              // used to bind them to the area portals by
...
enum emittype_t {
  ...
  emit_skylight, // directional light with no falloff (surface must trace to SKY
                 // texture)
  emit_quakelight, // linear falloff, non-lambertian
  emit_skyambient, // spherical light source with no falloff (surface must trace
                   // to SKY texture)
};
...
inline void dwateroverlay_t::SetRenderOrder(unsigned short order) {
  m_nFaceCountAndRenderOrder &= ~WATEROVERLAY_RENDER_ORDER_MASK;
  m_nFaceCountAndRenderOrder |=
      (order << (16 - WATEROVERLAY_RENDER_ORDER_NUM_BITS)); // leave 2 bits for
                                                            // render order.
}
...
typedef struct nameForDatadesc_dlightmappage_t // unnamed structs collide in the
                                               // datadesc macros
{
  DECLARE_BYTESWAP_DATADESC();
...
typedef struct nameForDatadesc_dlightmappageinfo_t // unnamed structs collide in
                                                   // the datadesc macros
{
  DECLARE_BYTESWAP_DATADESC();
